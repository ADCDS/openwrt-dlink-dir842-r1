...
# 2) <bspchip.h> lives in the mach dir; the kernel only adds the CONFIGURED platform's
...
#    BSP header that fights the kernel's own.
#    ★ FORCE-INCLUDE it rather than relying on the source reaching an #include.
#    Which branch takes <bspchip.h> depends on NOT_RTK_BSP / CONFIG_OPENWRT_SDK /
#    CONFIG_RTL_8197F, and those interact: every combination that fixed one error
#    class re-broke another (BSP_WLAN_* <-> pskb->__unused, measured repeatedly).
#    The symbols are unconditional inside the header, so pulling it in once up front
#    makes them available on EVERY branch and takes that variable out of the matrix.
#    ⚠ Do NOT force-include it with -include: that places it ahead of the kernel's
#    own headers and breaks arch/mips uaccess.h and thread_info.h ('invalid type
#    argument of ->', 'conflicting types for current_thread_info'). The -I above is
#    the correct mechanism; the source reaches #include <bspchip.h> on its own.
...
# 6) RTK_NL80211 — the vendor cfg80211/nl80211 layer. Its DEFINITIONS are gated on
#    this macro (struct rtl8192cd_priv::rtk at 8192cd.h:5638, the CFG80211_* enum at
#    :1057) while their USE sites in 8192cd_osdep.c are not, so without it the file
#    references members and enumerators that were never declared.
#    Enabling it is also the right direction for R4/G4: a WEXT-only driver cannot
#    share userspace with the mac80211/rtw88 stack already serving 5 GHz here, and
#    this layer is what gives the vendor driver a cfg80211 face.
...
# 8) BSP constants supplied directly. <bspchip.h> is reachable only on some branches
#    of the NOT_RTK_BSP / CONFIG_OPENWRT_SDK / CONFIG_RTL_8197F matrix, and
#    force-including it with -include lands it ahead of the kernel's own headers and
#    breaks arch/mips uaccess.h + thread_info.h. These are plain constants, so define
#    them and take the whole question out of the build.
...
CONFIG_SLOT_0_RFE_TYPE_10=y
CONFIG_SLOT_0_TX_BEAMFORMING=y
CONFIG_SOC_RFE_TYPE_0=y
CONFIG_SOC_WIFI=y
CONFIG_TXPWR_LMT=y
CONFIG_USE_PCIE_SLOT_0=y
...
ccflags-y += -DCONFIG_BAND_5G_ON_WLAN0
...
ccflags-y += -DCONFIG_RTL_5G_SLOT_0
