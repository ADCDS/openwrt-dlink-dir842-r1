...
/* DIAGNOSTIC INSTRUMENTATION -- port/main-6.18, M4 5GHz bring-up bug
 * (docs/PORT-MAIN-6.18-STATUS.md §3: standard wifi/UCI bring-up fails with
 * "netifd: radio0 (PID): command failed: Not supported (-122)" and the
 * exact failing nl80211 call was never pinned down by static source
 * tracing).
 *
 * NOTE for whoever picks this up: the investigation that motivated this
 * change hypothesized a blank/invalid RTL8822BE efuse MAC address as the
 * root cause (main's 04-rtw88-random-mac-blank-efuse.patch, never ported
 * to this branch, fixes exactly that on the kernel 4.14 product). That
 * hypothesis does NOT hold on this branch and the patch was deliberately
 * NOT ported: this port's own build tree already carries the equivalent
 * fix natively -- build_dir/target-mipsel_24kc_musl/linux-rtl819x_rtl8197f/
 * mac80211-regular/backports-7.2/drivers/net/wireless/realtek/rtw88/main.c,
 * rtw_chip_efuse_info_setup(),
 * already does
 *     if (!is_valid_ether_addr(efuse->addr)) { eth_random_addr(efuse->addr); ... }
 * unconditionally, upstream, on both this port's mac80211-backports-7.2
 * tree AND its in-kernel linux-6.18.44 copy of the same driver -- this is
 * mainline Linux having independently absorbed the same fix in the ~6
 * years between main's 2020-era hostapd/rtw88 snapshot and this port's
 * 2026-era one, not a gap in this branch. So the wiphy's permanent address
 * is never actually invalid by the time any interface gets created here,
 * and porting that patch forward would only add dead, redundant code.
 *
 * The real, still-open gap found while checking that hypothesis: this
 * function's caller (wdev_add(), just below) has always discarded the
 * return value of wdev_create() on the one live call path that reaches it
 * for a real radio bring-up (wdev.uc's iface_start()) -- so a failure
 * here, whatever actually causes it, has never been logged anywhere. Log
 * it, with the exact nl80211 command and attributes attempted, so the
 * next bench reproduction of the M4 bring-up failure captures which
 * command (NEW_INTERFACE vs SET_INTERFACE) was rejected, what iftype/
 * mac/vif_radio_mask were requested, and libnl's own decoded reason --
 * not just the bare errno netifd relays today. This is logging only:
 * wdev_create()'s return value and control flow are unchanged below. */
function wdev_create(phy, name, data)
{
	...
	if (error) {
		/* See the DIAGNOSTIC INSTRUMENTATION comment above this function. */
		warn(`wdev_create: ${cmd_name} failed for ${name} on phy ${phy} ` +
			`(wiphy=${phyidx}, iftype=${data.mode}` +
			(req.mac ? `, mac=${req.mac}` : "") +
			(req["4addr"] ? `, 4addr=${req["4addr"]}` : "") +
			(req.vif_radio_mask != null ? `, vif_radio_mask=${req.vif_radio_mask}` : "") +
			`): ${error}\n`);
		return error;
	}
