...
endef
# DIR-842 M5: QCA99X0 board-2.bin lives only on the decommissioned
# source.codeaurora.org (DNS dead, OpenWrt mirrors 404), so this unconditional
# download fails linux-firmware/compile for EVERY build. This box has no ath10k
# radio and ath10k-firmware-qca99x0 is not selected (the only consumer is that
# package's install recipe below, which never runs), so drop the download.
