...
/*chester modified for bt-coexist*/
//typedef struct rtl8192cd_priv RTL8192CD_PRIV, *PRTL8192CD_PRIV;
...
enum NETWORK_TYPE {
	...
	WIRELESS_11N = BIT(3), /* tx: MCS only, rx: MCS & cck, hw: MCS & cck */ 
...
struct tcp_session {
	...
	unsigned int		is_delay_start_ok; 
};
#endif
...
struct stat_info {
    ...
    unsigned int		atm_C2H_tx_time; //Firmware report real transmission time//ms
   ...
   struct	dot11v_stat_info wnm;
	...
	unsigned short		tcpack_to_cnt1;
	unsigned short		tcpack_to_cnt2;
	unsigned short		tcpack_full_cnt;
	unsigned short		tcpack_buf_cnt;
#endif
...
//#if !defined(USE_OUT_SRC) || defined(_OUTSRC_COEXIST)
	...
	//TCP_ACK_ACC
	unsigned int		rx_pkts2; //rx pkt to br //rx tcp pkt
	unsigned int		tcpack_pkts;
	unsigned int		tcpack_pkts_sent;
	unsigned int		tcpack_drop;
...
struct disconnect_info {
	...
	unsigned char	branch_code;
};

struct assoc_stas_info {
	unsigned long	first_connect_time;
	unsigned long	connect_succ_time;
	unsigned int	connect_count;
	unsigned int	connect_fail_count;
	unsigned char	addr[MACADDRLEN];
	unsigned char	last_connect_fail_code;
	unsigned char	rssi;
};
...
/*cfg p2p cfg p2p*/	
	...
	unsigned int			kfree_reg_0xde_pathA;
	unsigned int			kfree_reg_0xde_pathB;
#endif
...
//#endif
	...
	struct monitor_station			monitor_sta[MAX_MONITOR_STA];	// records sta rssi info
...
#if 1//defined(SUPPORT_TX_AMSDU) || defined (P2P_SUPPORT)
...
#ifdef ENABLE_BT_COEXIST
	struct bt_coexist_common_data bt_coex_comm_data;
#endif
...
typedef struct rtl8192cd_priv {
...
#ifdef _DEBUG_RTL8192CD_
	/* BIT(31): HWRTSEn, BIT(30): RTSEn, BIT(29): CTS2Self, BIT(28)~BIT(0): reason */
	...
	struct list_head		addps_list;
#endif
	struct disconnect_info	del_sta_info;	// AP actively deletes STA
	struct disconnect_info	leave_sta_info;	// STA actively leaves AP
	struct timer_list		connect_sta_info_timer;
	struct assoc_stas_info	connect_sta_info[MAX_CONNECT_STA_INFO];
	unsigned int			connect_sta_info_num;
#if defined(SMP_SYNC) 
	//TCP_ACK_ACC
...
#ifdef CONFIG_SAPPS
    unsigned char enable_SAPPS;
#endif
#ifdef CONFIG_32K
    unsigned char enable_32K;
...
#endif
	unsigned char dev_init_reason_code;
	unsigned char dev_init_return_code;
...
#define IS_TCP_PROTO(mac)	((mac[12]==0x08) && (mac[13]==0x00) && (mac[23]==0x06) && (mac[47]==0x10))
#if defined(__OSK__)
...
//TCP_ACK_ACC
#define IPV4_TCP_ACK_PACKET(eth_packet)  ( eth_packet[12] ==0x08 	&& eth_packet[13] ==0x00 \
													&&  eth_packet[23] ==0x06 \
													&&  eth_packet[47] ==0x10 \
													&&  (ntohs(*(unsigned short *)(eth_packet+16)) == (((*(unsigned char *)(eth_packet+14)&0xF)<<2)+(*(unsigned char *)(eth_packet+46)>>4<<2))))
