...
*/
	...
	if (GET_CHIP_VER(priv)==VERSION_8197F){
		if (eRFPath == RF92CD_PATH_A){
			PHY_SetBBReg(priv, 0x4c, bMaskDWord, 0x05e28282);   
			PHY_SetBBReg(priv, 0x64, bMaskDWord, 0x16270083);
			PHY_SetBBReg(priv, 0x78, bMaskDWord, 0x070102fb);
			PHY_SetBBReg(priv, 0x88c, bMaskDWord, 0xccf000c0);			
			PHY_SetRFReg(priv, eRFPath, 0x0, bMask20Bits, 0x20000);
			PHY_SetRFReg(priv, eRFPath, 0x57, bMask20Bits, 0x7f002);
		}else {
			PHY_SetBBReg(priv, 0x4c, bMaskDWord, 0x05e28282);   
			PHY_SetBBReg(priv, 0x64, bMaskDWord, 0x16270080);
			PHY_SetBBReg(priv, 0x78, bMaskDWord, 0x070102fb);
			PHY_SetBBReg(priv, 0x88c, bMaskDWord, 0xccf000c0);
			PHY_SetRFReg(priv, eRFPath, 0x0, bMask20Bits, 0x20000);
			PHY_SetRFReg(priv, eRFPath, 0x57, bMask20Bits, 0x7f002);
