...
#define SMP_TRY_LOCK_ASOC_LIST(__x__, __y__)//TCP_ACK_ACC
...
#define SMP_TRY_LOCK_ASOC_LIST(__x__, __y__)	_enter_critical_mutex(&priv->asoc_list_lock, (__x__))//TCP_ACK_ACC
...
#define SMP_TRY_LOCK_ASOC_LIST(__x__, __y__)	do { spin_lock_bh(&priv->asoc_list_lock); (void)(__x__); } while (0)//TCP_ACK_ACC
...
#define SMP_LOCK_ASOC_LIST(__x__) \
	do { \
		...
		priv->asoc_list_lock_owner = smp_processor_id(); \
	} while(0)
...
#ifdef CONFIG_IEEE80211R
#define SMP_LOCK_FT_R0KH(__x__)			spin_lock_irqsave(&priv->ft_r0kh_lock, (__x__))
#define SMP_UNLOCK_FT_R0KH(__x__)		spin_unlock_irqrestore(&priv->ft_r0kh_lock, (__x__))

#define SMP_LOCK_FT_R1KH(__x__)			spin_lock_irqsave(&priv->ft_r1kh_lock, (__x__))
#define SMP_UNLOCK_FT_R1KH(__x__)		spin_unlock_irqrestore(&priv->ft_r1kh_lock, (__x__))
#endif
#ifdef RTK_NL80211
...
#define IS_TDMA_DIG_CHIP(priv)		((GET_CHIP_VER(priv) == VERSION_8822B) || (GET_CHIP_VER(priv) == VERSION_8197F) || ((GET_CHIP_VER(priv) == VERSION_8814A) && priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G))
