...
#include "./btcoexist/hal/hal_btcoexist.h"
#include "./btcoexist/btcoexist_cmd.h"

#define RTW_DBG printk
#define RTW_INFO printk

void rtw_btcoex_Initialize(BT_COEX_HAL_ADAPTER *padapter)
{
	hal_btcoex_Initialize(padapter);
}

void rtw_btcoex_PowerOnSetting(BT_COEX_HAL_ADAPTER *padapter)
{
	hal_btcoex_PowerOnSetting(padapter);
}

void rtw_btcoex_PreLoadFirmware(BT_COEX_HAL_ADAPTER *padapter)
{
	hal_btcoex_PreLoadFirmware(padapter);
}

void rtw_btcoex_HAL_Initialize(BT_COEX_HAL_ADAPTER *padapter, u8 bWifiOnly)
{
	hal_btcoex_InitHwConfig(padapter, bWifiOnly);
}

void rtw_btcoex_IpsNotify(BT_COEX_HAL_ADAPTER *padapter, u8 type)
{
	...
	//	return;
	if (!IS_BT_COEXIST_AVALIABLE(padapter))
			return;

	hal_btcoex_IpsNotify(padapter, type);
}

void rtw_btcoex_LpsNotify(BT_COEX_HAL_ADAPTER *padapter, u8 type)
{
#if 0	
	PHAL_DATA_TYPE	pHalData;
...
void rtw_btcoex_ScanNotify(BT_COEX_HAL_ADAPTER *padapter, u8 type)
{
	//PHAL_DATA_TYPE	pHalData;
#ifdef CONFIG_BT_COEXIST_SOCKET_TRX
	struct bt_coex_info *pcoex_info = &padapter->pshare->bt_coex_comm_data.coex_info;
...
void rtw_btcoex_ConnectNotify(BT_COEX_HAL_ADAPTER *padapter, u8 action)
{
#if 0	
	PHAL_DATA_TYPE	pHalData;
...
void rtw_btcoex_MediaStatusNotify(BT_COEX_HAL_ADAPTER *padapter, u8 mediaStatus)
{
	if (!IS_BT_COEXIST_AVALIABLE(padapter))
		return;
#if 0	
...
void rtw_btcoex_SpecialPacketNotify(BT_COEX_HAL_ADAPTER *padapter, u8 pktType)
{
	//PHAL_DATA_TYPE	pHalData;
...
void rtw_btcoex_IQKNotify(BT_COEX_HAL_ADAPTER *padapter, u8 state)
{
	//PHAL_DATA_TYPE	pHalData;
...
void rtw_btcoex_BtInfoNotify(BT_COEX_HAL_ADAPTER *padapter, u8 length, u8 *tmpBuf)
{
	//PHAL_DATA_TYPE	pHalData;
...
void rtw_btcoex_BtMpRptNotify(BT_COEX_HAL_ADAPTER *padapter, u8 length, u8 *tmpBuf)
{
/*	
	PHAL_DATA_TYPE	pHalData;
...
void rtw_btcoex_SuspendNotify(BT_COEX_HAL_ADAPTER *padapter, u8 state)
{
	//PHAL_DATA_TYPE	pHalData;
...
void rtw_btcoex_HaltNotify(BT_COEX_HAL_ADAPTER *padapter)
{
...
void rtw_btcoex_ScoreBoardStatusNotify(BT_COEX_HAL_ADAPTER *padapter, u8 length, u8 *tmpBuf)
{
	if (!IS_BT_COEXIST_AVALIABLE(&padapter))
		return;
...
void rtw_btcoex_SwitchBtTRxMask(BT_COEX_HAL_ADAPTER *padapter)
{
	hal_btcoex_SwitchBtTRxMask(padapter);
}

void rtw_btcoex_Switch(BT_COEX_HAL_ADAPTER *padapter, u8 enable)
{
	hal_btcoex_SetBTCoexist(padapter, enable);
}

u8 rtw_btcoex_IsBtDisabled(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_IsBtDisabled(padapter);
}

void rtw_btcoex_Handler(BT_COEX_HAL_ADAPTER *padapter)
{
	//PHAL_DATA_TYPE	pHalData;
...
s32 rtw_btcoex_IsBTCoexRejectAMPDU(BT_COEX_HAL_ADAPTER *padapter)
{
	s32 coexctrl;
...
s32 rtw_btcoex_IsBTCoexCtrlAMPDUSize(BT_COEX_HAL_ADAPTER *padapter)
{
	s32 coexctrl;
...
u32 rtw_btcoex_GetAMPDUSize(BT_COEX_HAL_ADAPTER *padapter)
{
	u32 size;
...
void rtw_btcoex_SetManualControl(BT_COEX_HAL_ADAPTER *padapter, u8 manual)
{
	if (_TRUE == manual)
...
u8 rtw_btcoex_1Ant(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_1Ant(padapter);
}

u8 rtw_btcoex_IsBtControlLps(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_IsBtControlLps(padapter);
}

u8 rtw_btcoex_IsLpsOn(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_IsLpsOn(padapter);
}

u8 rtw_btcoex_RpwmVal(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_RpwmVal(padapter);
}

u8 rtw_btcoex_LpsVal(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_LpsVal(padapter);
}

u32 rtw_btcoex_GetRaMask(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_GetRaMask(padapter);
}

void rtw_btcoex_RecordPwrMode(BT_COEX_HAL_ADAPTER *padapter, u8 *pCmdBuf, u8 cmdLen)
{
	hal_btcoex_RecordPwrMode(padapter, pCmdBuf, cmdLen);
}

void rtw_btcoex_DisplayBtCoexInfo(BT_COEX_HAL_ADAPTER *padapter, u8 *pbuf, u32 bufsize)
{
	hal_btcoex_DisplayBtCoexInfo(padapter, pbuf, bufsize);
}

void rtw_btcoex_SetDBG(BT_COEX_HAL_ADAPTER *padapter, u32 *pDbgModule)
{
	hal_btcoex_SetDBG(padapter, pDbgModule);
}

u32 rtw_btcoex_GetDBG(BT_COEX_HAL_ADAPTER *padapter, u8 *pStrBuf, u32 bufSize)
{
	return hal_btcoex_GetDBG(padapter, pStrBuf, bufSize);
}

u8 rtw_btcoex_IncreaseScanDeviceNum(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_IncreaseScanDeviceNum(padapter);
}

u8 rtw_btcoex_IsBtLinkExist(BT_COEX_HAL_ADAPTER *padapter)
{
	return hal_btcoex_IsBtLinkExist(padapter);
}

void rtw_btcoex_SetBtPatchVersion(BT_COEX_HAL_ADAPTER *padapter, u16 btHciVer, u16 btPatchVer)
{
	hal_btcoex_SetBtPatchVersion(padapter, btHciVer, btPatchVer);
}

void rtw_btcoex_SetHciVersion(BT_COEX_HAL_ADAPTER *padapter, u16 hciVersion)
{
...
void rtw_btcoex_pta_off_on_notify(BT_COEX_HAL_ADAPTER *padapter, u8 bBTON)
{
 ...
 * ================================================== */
void rtw_btcoex_rx_ampdu_apply(BT_COEX_HAL_ADAPTER *padapter)
{
	//rtw_rx_ampdu_apply(padapter);
}

void rtw_btcoex_LPS_Enter(BT_COEX_HAL_ADAPTER *padapter)
{
/*	
...
void rtw_btcoex_LPS_Leave(BT_COEX_HAL_ADAPTER *padapter)
{
/*	
	struct pwrctrl_priv *pwrpriv;
...
u16 rtw_btcoex_btreg_read(BT_COEX_HAL_ADAPTER *padapter, u8 type, u16 addr, u32 *data)
{
	return hal_btcoex_btreg_read(padapter, type, addr, data);
}

u16 rtw_btcoex_btreg_write(BT_COEX_HAL_ADAPTER *padapter, u8 type, u16 addr, u16 val)
{
	return hal_btcoex_btreg_write(padapter, type, addr, val);
}

u8 rtw_btcoex_get_bt_coexist(BT_COEX_HAL_ADAPTER *padapter)
{
	struct bt_coexist_common_data bt_coex_comm_data = padapter->pshare->bt_coex_comm_data;
	
	return bt_coex_comm_data.EEPROMBluetoothCoexist;
}

u8 rtw_btcoex_get_chip_type(BT_COEX_HAL_ADAPTER *padapter)
{
	struct bt_coexist_common_data bt_coex_comm_data = padapter->pshare->bt_coex_comm_data;

	return bt_coex_comm_data.EEPROMBluetoothType;

}

u8 rtw_btcoex_get_pg_ant_num(BT_COEX_HAL_ADAPTER *padapter)
{
	struct bt_coexist_common_data bt_coex_comm_data = padapter->pshare->bt_coex_comm_data;

	return bt_coex_comm_data.EEPROMBluetoothAntNum == Ant_x2 ? 2 : 1;

}

u8 rtw_btcoex_get_pg_single_ant_path(BT_COEX_HAL_ADAPTER *padapter)
{
...
u8 rtw_btcoex_get_pg_rfe_type(BT_COEX_HAL_ADAPTER *padapter)
{
	//HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
...
u8 rtw_btcoex_is_tfbga_package_type(BT_COEX_HAL_ADAPTER *padapter)
{
/*	
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
...
u8 rtw_btcoex_get_ant_div_cfg(BT_COEX_HAL_ADAPTER *padapter)
{
	//PHAL_DATA_TYPE pHalData;
...
#ifdef CONFIG_BT_COEXIST_SOCKET_TRX
BT_COEX_HAL_ADAPTER *pbtcoexadapter; /* = NULL; */ /* do not initialise globals to 0 or NULL */
u8 rtw_btcoex_btinfo_cmd(BT_COEX_HAL_ADAPTER *adapter, u8 *buf, u16 buf_len)
{
...
#define BTINFO_BT_AUTO_RPT 0x27
	...
	rtw_btcoex_BtInfoNotify(adapter,len+1,&buf[1]); //+wishchen
...
u8 rtw_btcoex_send_event_to_BT(BT_COEX_HAL_ADAPTER *padapter, u8 status,  u8 event_code, u8 opcode_low, u8 opcode_high, u8 *dbg_msg)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
...
u8 rtw_btcoex_parse_BT_info_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
...
*/
	...
	if (BT_INFO_LENGTH != btInfoLen) {
		...
		RTW_DBG("Error BT Info Length: %d\n", btInfoLen);
		/* return _FAIL; */
	} else
#endif
	{
...
u8 rtw_btcoex_parse_BT_patch_ver_info_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS status = HCI_STATUS_SUCCESS;
...
u8 rtw_btcoex_parse_HCI_Ver_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS status = HCI_STATUS_SUCCESS;
...
u8 rtw_btcoex_parse_WIFI_scan_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS status = HCI_STATUS_SUCCESS;
...
u8 rtw_btcoex_parse_HCI_link_status_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS	status = HCI_STATUS_SUCCESS;
	struct bt_coex_info	*pcoex_info = &padapter->pshare->bt_coex_comm_data.coex_info;
	PBT_MGNT	pBtMgnt = &pcoex_info->BtMgnt;
...
u8 rtw_btcoex_parse_HCI_BT_coex_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8 len = 0, tx_event_length = 0;
...
u8 rtw_btcoex_parse_HCI_BT_operation_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len = 0, tx_event_length = 0;
	...
	case HCI_BT_OP_BT_DEV_DISABLE:
		...
		break;
	default:
		RTW_DBG("[bt operation] : Unknown, error!!\n");
		break;
	}
...
u8 rtw_btcoex_parse_BT_AFH_MAP_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len = 0, tx_event_length = 0;
...
u8 rtw_btcoex_parse_BT_register_val_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
...
u8 rtw_btcoex_parse_HCI_BT_abnormal_notify_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len = 0, tx_event_length = 0;
...
u8 rtw_btcoex_parse_HCI_query_RF_status_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len = 0, tx_event_length = 0;
 ...
 *	 |	OCF			             |	   OGF       | */
void rtw_btcoex_parse_hci_extend_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 len, const u16 hci_OCF)
{
	...
	case HCI_BT_REGISTER_VALUE_NOTIFY:
		...
		break;
	default:
		RTW_DBG("ERROR!!! Unknown OCF: %x\n", hci_OCF);
		break;

	}
}

void rtw_btcoex_parse_hci_cmd(BT_COEX_HAL_ADAPTER *padapter, u8 *pcmd, u16 len)
{
...
u16 rtw_btcoex_parse_recv_data(u8 *msg, u8 msg_size)
{
	...
	if (!memcmp(cmp_msg1, msg, msg_size)) {
		res = RX_ATTEND_ACK;
	} else if (!memcmp(cmp_msg2, msg, msg_size)) {
		res = RX_LEAVE_ACK;
	} else if (!memcmp(cmp_msg3, msg, msg_size)) {
		res = RX_BT_LEAVE;
	} else if (!memcmp(cmp_msg4, msg, msg_size)) {
		res = RX_INVITE_REQ;
	} else if (!memcmp(cmp_msg5, msg, msg_size)){
		res = RX_ATTEND_REQ;
	}else if (!memcmp(cmp_msg6, msg, msg_size)){
		res = RX_INVITE_RSP;
	}
	else {
		res = OTHER;
...
void rtw_btcoex_recvmsgbysocket(void *data)
{
	...
	pcoex_info = &pbtcoexadapter->pshare->bt_coex_comm_data.coex_info;
	...
	while (len > 0) {
		...
		default:
			//RTW_DBG("Other commands!\n");
			if (_TRUE == pcoex_info->BT_attend)
				rtw_btcoex_parse_hci_cmd(pbtcoexadapter, recv_data, recv_length);
			else
				RTW_DBG("ERROR!! BT is UP\n");
			break;
...
u8 rtw_btcoex_sendmsgbysocket(BT_COEX_HAL_ADAPTER *padapter, u8 *msg, u8 msg_size, bool force)
{
	u8 error;
	...
	RTW_DBG("   >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>	WIFI_TO_BT,msg:%s , size=%d\n",msg, msg_size);
...
u8 rtw_btcoex_create_kernel_socket(BT_COEX_HAL_ADAPTER *padapter)
{
...
void rtw_btcoex_close_kernel_socket(BT_COEX_HAL_ADAPTER *padapter)
{
	struct bt_coex_info *pcoex_info = &padapter->pshare->bt_coex_comm_data.coex_info;
...
void rtw_btcoex_init_socket(BT_COEX_HAL_ADAPTER *padapter)
{
...
void rtw_btcoex_close_socket(BT_COEX_HAL_ADAPTER *padapter)
{
	struct bt_coex_info *pcoex_info = &padapter->pshare->bt_coex_comm_data.coex_info;
...
void rtw_btcoex_SendEventExtBtCoexControl(BT_COEX_HAL_ADAPTER *padapter, u8 bNeedDbgRsp, u8 dataLen, void *pData)
{
	u8			len = 0, tx_event_length = 0;
	u8 			localBuf[32] = "";
	u8			*pRetPar;
...
/* Porting from Windows team */
void rtw_btcoex_SendEventExtBtInfoControl(BT_COEX_HAL_ADAPTER *padapter, u8 dataLen, void *pData)
{
	rtw_HCI_event *pEvent;
	u8			*pRetPar;
	u8			len = 0, tx_event_length = 0;
...
void rtw_btcoex_SendScanNotify(BT_COEX_HAL_ADAPTER *padapter, u8 scanType)
{
	u8	len = 0, tx_event_length = 0;
	u8 	localBuf[7] = "";
	u8	*pRetPar;
