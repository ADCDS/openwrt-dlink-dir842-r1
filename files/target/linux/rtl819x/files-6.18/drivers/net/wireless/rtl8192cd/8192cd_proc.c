...
#endif
{
	...
	//rtw_btcoex_GetDBG(padapter, buffer, 512);
...
extern void hal_btcoex_SetDBG(PADAPTER padapter, u32 *pDbgModule);
...
extern void hal_btcoex_DisplayBtCoexInfo(PADAPTER padapter, u8 *pbuf, u32 bufsize);
#ifdef CONFIG_RTL_PROC_NEW
...
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_ccx_read(struct seq_file *s, void *data)
#else
static int rtl8192cd_proc_ccx_read(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif

{
	...
	if (phydm_checkCLMready(pDM_Odm) ){ 		/* --Getting CLM info previous time--*/	
		clm_count = phydm_getCLMresult(pDM_Odm);	
	}
	 if(clm_count > 0){
		priv->pmib->miscEntry.clm = (clm_count*100)>>16;
	}		
	/*NHM*/
	 if (phydm_checkNHMready(pDM_Odm)){ 	 /* --Getting CLM info previous time--*/ 
			 phydm_getNHMresult(pDM_Odm);
	 }
 
	PRINT_ONE("  ccx result...", "%s", 1);	
	PRINT_SINGL_ARG("    CLM : ", priv->pmib->miscEntry.clm, "%d");
	PRINT_SINGL_ARG("    NHM : ", priv->pmib->miscEntry.nhm, "%d");
...
#ifdef CONFIG_RTL_PROC_NEW
static int rtl8192cd_proc_ccx_write(struct file *file, const char *buffer,
...

{
	...
	if (buffer && !copy_from_user(tmp, buffer, 4)) {
		if(0 == (tmp[0]-'1')){
			...
			/*note! make sure 		phydm_CLMInit(ODMPTR, CLM_SAMPLE_NUM2);	at osdep has be executed*/
			phydm_CLMtrigger(ODMPTR);/* trigger next time CLM */
		
			/*---------------[NHM]---------------------------*/
			//noise_histogram_measurement(priv);

			/*[NHM setting and trigger]*/
...
#ifdef CONFIG_RTL_PROC_NEW
int rtl8192cd_proc_debug_statistic(struct seq_file *s, void *data)
#else	
static int rtl8192cd_proc_debug_statistic(char *buf, char **start, off_t offset,
			int length, int *eof, void *data)
#endif	
{
	struct net_device *dev = PROC_GET_DEV();
	...
	PRINT_ONE("  AP statistic...", "%s", 1);
#ifdef _DEBUG_RTL8192CD_
	PRINT_ONE("    rts_enable_reason: ", "%s", 0);
	for(i = 0; i < 32; i++) {
		if(priv->rts_enable_reason & 1 << (31 - i)) {
			PRINT_ONE("1", "%s", 0);
		} else {
			PRINT_ONE("0", "%s", 0);
		}
	...
	PRINT_ONE("  Primary Channel Statistic...", "%s", 1);
	PRINT_SINGL_ARG("    ap_counts:    ", priv->pshare->currch_info.ap_count, "%u");
	PRINT_SINGL_ARG("    max_ap_rssi:  ", priv->pshare->currch_info.max_rssi, "%u");

	PRINT_ONE("  Last Auto Channel Select Statistic...", "%s", 1);
	for (i = 0; i < priv->available_chnl_num; i++) {
		PRINT_ONE("    ch: ", "%s", 0);
		PRINT_ONE(priv->available_chnl[i], "%3u", 0);
		PRINT_ONE(", ap_counts: ", "%s", 0);
		PRINT_ONE(priv->pshare->acs_info[i].ap_count, "%3u", 0);
		PRINT_ONE(", max_ap_rssi: ", "%s", 0);
		PRINT_ONE(priv->pshare->acs_info[i].max_rssi, "%3u", 1);
	}

	PRINT_ONE("  Last Deleted Station Info...", "%s", 1);
	PRINT_ARRAY_ARG("    addr: ", priv->del_sta_info.addr, "%02x", MACADDRLEN);
#ifdef RTK_AC_SUPPORT  //vht rate
	if (priv->del_sta_info.tx_rate >= VHT_RATE_ID) {
		PRINT_ONE("    tx_rate: VHT NSS", "%s", 0);
		PRINT_ONE(((priv->del_sta_info.tx_rate - VHT_RATE_ID) / 10) + 1, "%u", 0);
		PRINT_ONE("-MCS", "%s", 0);
		PRINT_ONE((priv->del_sta_info.tx_rate - VHT_RATE_ID) % 10, "%u", 1);
	}
	else
#endif
	if (is_MCS_rate(priv->del_sta_info.tx_rate)) {
		PRINT_SINGL_ARG("    tx_rate: MCS", (priv->del_sta_info.tx_rate - HT_RATE_ID), "%u");
	}
	else {
		PRINT_SINGL_ARG("    tx_rate: ", (priv->del_sta_info.tx_rate / 2), "%u");
	}

#ifdef RTK_AC_SUPPORT
	if (priv->del_sta_info.rx_rate >= VHT_RATE_ID) {
		PRINT_ONE("    rx_rate: VHT NSS", "%s", 0);
		PRINT_ONE(((priv->del_sta_info.rx_rate - VHT_RATE_ID) / 10) + 1, "%u", 0);
		PRINT_ONE("-MCS", "%s", 0);
		PRINT_ONE((priv->del_sta_info.rx_rate - VHT_RATE_ID) % 10, "%u", 1);
	}
	else
#endif
	if (is_MCS_rate(priv->del_sta_info.rx_rate)) {
		PRINT_SINGL_ARG("    rx_rate: MCS", (priv->del_sta_info.rx_rate - HT_RATE_ID), "%u");
	}
	else {
		PRINT_SINGL_ARG("    rx_rate: ", (priv->del_sta_info.rx_rate / 2), "%u");
	}
	
	PRINT_SINGL_ARG("    tx_tp: ", priv->del_sta_info.tx_tp, "%u");
	PRINT_SINGL_ARG("    rx_tp: ", priv->del_sta_info.rx_tp, "%u");
	PRINT_SINGL_ARG("    rssi: ", priv->del_sta_info.rssi, "%u");
	PRINT_SINGL_ARG("    reason_code: ", priv->del_sta_info.reason_code, "%u");
	PRINT_SINGL_ARG("    branch_code: ", priv->del_sta_info.branch_code, "%u");
	
	PRINT_ONE("  Last Leave Station Info...", "%s", 1);
	PRINT_ARRAY_ARG("    addr: ", priv->leave_sta_info.addr, "%02x", MACADDRLEN);
#ifdef RTK_AC_SUPPORT  //vht rate
	if (priv->leave_sta_info.tx_rate >= VHT_RATE_ID) {
		PRINT_ONE("    tx_rate: VHT NSS", "%s", 0);
		PRINT_ONE(((priv->leave_sta_info.tx_rate - VHT_RATE_ID) / 10) + 1, "%u", 0);
		PRINT_ONE("-MCS", "%s", 0);
		PRINT_ONE((priv->leave_sta_info.tx_rate - VHT_RATE_ID) % 10, "%u", 1);
	}
	else
#endif
	if (is_MCS_rate(priv->leave_sta_info.tx_rate)) {
		PRINT_SINGL_ARG("    tx_rate: MCS", (priv->leave_sta_info.tx_rate - HT_RATE_ID), "%u");
	}
	else {
		PRINT_SINGL_ARG("    tx_rate: ", (priv->leave_sta_info.tx_rate / 2), "%u");
	}

#ifdef RTK_AC_SUPPORT
	if (priv->leave_sta_info.rx_rate >= VHT_RATE_ID) {
		PRINT_ONE("    rx_rate: VHT NSS", "%s", 0);
		PRINT_ONE(((priv->leave_sta_info.rx_rate - VHT_RATE_ID) / 10) + 1, "%u", 0);
		PRINT_ONE("-MCS", "%s", 0);
		PRINT_ONE((priv->leave_sta_info.rx_rate - VHT_RATE_ID) % 10, "%u", 1);
	}
	else
#endif
	if (is_MCS_rate(priv->leave_sta_info.rx_rate)) {
		PRINT_SINGL_ARG("    rx_rate: MCS", (priv->leave_sta_info.rx_rate - HT_RATE_ID), "%u");
	}
	else {
		PRINT_SINGL_ARG("    rx_rate: ", (priv->leave_sta_info.rx_rate / 2), "%u");
	}
	
	PRINT_SINGL_ARG("    tx_tp: ", priv->leave_sta_info.tx_tp, "%u");
	PRINT_SINGL_ARG("    rx_tp: ", priv->leave_sta_info.rx_tp, "%u");
	PRINT_SINGL_ARG("    rssi: ", priv->leave_sta_info.rssi, "%u");
	PRINT_SINGL_ARG("    reason_code: ", priv->leave_sta_info.reason_code, "%u");
	PRINT_SINGL_ARG("    branch_code: ", priv->leave_sta_info.branch_code, "%u");

	PRINT_ONE("  Station Connect Statistic...", "%s", 1);
	PRINT_ONE("    Station Num:", "%s", 0);
	PRINT_ONE(priv->connect_sta_info_num, "%u", 1);
	
	for (i = 0; i < priv->connect_sta_info_num; i++) {
			PRINT_ONE("    Station", "%s", 0);
			PRINT_ONE(i, " %2u:", 1);
			PRINT_ARRAY_ARG("      addr:                 ", priv->connect_sta_info[i].addr, "%02x", MACADDRLEN);
			PRINT_SINGL_ARG("      connect count:        ", priv->connect_sta_info[i].connect_count, "%u");
			PRINT_SINGL_ARG("      connect fail count:   ", priv->connect_sta_info[i].connect_fail_count, "%u");
			PRINT_SINGL_ARG("      first connect time:   ", priv->connect_sta_info[i].first_connect_time, "%lu");
			PRINT_SINGL_ARG("      connect success time: ", priv->connect_sta_info[i].connect_succ_time, "%lu");
			PRINT_SINGL_ARG("      reason:               ", priv->connect_sta_info[i].last_connect_fail_code, "%u");
			PRINT_SINGL_ARG("      rssi:                 ", priv->connect_sta_info[i].rssi, "%u");
	}
...
#endif	
{
	...
	PRINT_SINGL_ARG("    veriwave_noisy_mode: ", priv->pshare->veriwave_noisy_mode, "%d");
	...
	PRINT_ONE(priv->ext_stats.swq_drop_pkt, "drop: %lu )", 1);
#endif
...
//	CHECK_LEN_E;
	size = proc_get_btcoex_info(pTmp, start, offset, length, eof, data);
	CHECK_LEN_E;
#endif
...
#endif			
{
	...
	if (is_MCS_rate(pstat->current_tx_rate)) {
		...
		rate = (unsigned char *)MCS_DATA_RATEStr[pstat->tx_bw&0x01][(pstat->ht_current_tx_info&BIT(1))?1:0][(pstat->current_tx_rate - HT_RATE_ID)];
		PRINT_ONE(rate, " %s", 1);
		CHECK_LEN_B;
	}
	else
...
/* Hotspot 2.0 Release 1 */
	...
	//if (priv->pmib->miscEntry.tcpack_acc) {
		...
		PRINT_SINGL_ARG("	 tcp_ses_cnt: ", cnt, "%d");
		PRINT_SINGL_ARG("	 tcpack_acc_en: ", pstat->tcpack_acc_en, "%u");
		PRINT_SINGL_ARG("	 tcpack_use_hw_timer: ", priv->pmib->miscEntry.tcpack_use_hw_timer, "%d ");

		if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G){
			PRINT_SINGL_ARG("	 tcpack_to: ", priv->pmib->miscEntry.tcpack_to_2g, "%d");
		} else {
			PRINT_SINGL_ARG("	 tcpack_to: ", priv->pmib->miscEntry.tcpack_to, "%d");
		}

		PRINT_SINGL_ARG("	 tcpack_to_cnt1(timer): ", pstat->tcpack_to_cnt1, "%u");
		PRINT_SINGL_ARG("	 tcpack_to_cnt2(enqeue): ", pstat->tcpack_to_cnt2, "%u");
		PRINT_SINGL_ARG("	 tcpack_full_cnt: ", pstat->tcpack_full_cnt, "%u");
		PRINT_SINGL_ARG("	 tcpack_buf_cnt: ", pstat->tcpack_buf_cnt, "%u");
		PRINT_SINGL_ARG("    tcpack_pkts: ", pstat->tcpack_pkts, "%u");
		PRINT_SINGL_ARG("    tcpack_pkts_sent: ", pstat->tcpack_pkts_sent, "%u");
		PRINT_SINGL_ARG("    tcpack_pkts_diff: ", pstat->tcpack_pkts - pstat->tcpack_pkts_pre, "%u");
		PRINT_SINGL_ARG("    tcpack_rx_pkts2: ", pstat->rx_pkts2, "%u");
		PRINT_SINGL_ARG("    tcpack_rx_pkts2_diff: ", pstat->rx_pkts2 - pstat->rx_pkts2_pre, "%u");
		PRINT_SINGL_ARG("    tcpack_drop: ", pstat->tcpack_drop, "%u");
		PRINT_SINGL_ARG("    tcpack_drop_ratio: ", pstat->tcpack_drop*100/pstat->tcpack_pkts, "%d %%");		
	//}
#endif
	...
	CHECK_LEN_B;
	PRINT_SINGL_ARG("	 mimo num: ", pstat->mimo_num, "%d ");
	CHECK_LEN_B;
	PRINT_ONE("", "%s", 1);
...
#endif			
{
	...
	PRINT_SINGL_ARG("    rx_reuse_frag:      ", priv->ext_stats.rx_reuse_frag, "%lu");	
	...
	PRINT_SINGL_ARG("    tx_data_pkts:  ", priv->ext_stats.tx_data_pkts, "%lu");
	PRINT_SINGL_ARG("    rx_data_pkts:  ", priv->ext_stats.rx_data_pkts, "%lu");
	PRINT_SINGL_ARG("    tx_data_pkts_diff:  ", priv->ext_stats.NumTxOkInPeriod, "%lu");
	PRINT_SINGL_ARG("    rx_data_pkts_diff:  ", priv->ext_stats.NumRxOkInPeriod, "%lu");
#endif
...
PRINT_SINGL_ARG("    dev_init_reason_code: ", priv->dev_init_reason_code, "%d");
PRINT_SINGL_ARG("    dev_init_return_code: ", priv->dev_init_return_code, "%d ");
#if (MU_BEAMFORMING_SUPPORT == 1)
...
/* Hotspot 2.0 Release 1 */
	...
	PRINT_SINGL_ARG("	 TC concurrent mode: ", priv->pshare->tc_msta_dual, "%d");
#endif
	PRINT_SINGL_ARG("    adaptivity_enable: ", priv->pshare->rf_ft_var.adaptivity_enable, "%d");
				...
				unsigned long count, void *data)
{
	...
	if (buffer && !copy_from_user(enable, buffer, sizeof(enable))) {
		...
		if(priv->vdev_enabled^vChar)
		{
			...
			if(vChar){
				if(vChar>WLAN_VDEV_FOR_V4_V6)
				{
					printk("[%s:%d] invalid value %d\n", __FUNCTION__, __LINE__, vChar);
					return -1;
				}
...
#endif			
{
	...
	RTK_DECLARE_READ_PROC_FOPS(rtl8192cd_proc_debug_statistic);
...
char tmp_log[128];
	...
	RTK_DECLARE_READ_WRITE_PROC_FOPS(rtl8192cd_proc_ccx_read, rtl8192cd_proc_ccx_write);
...
void MDL_DEVINIT rtl8192cd_proc_init(struct net_device *dev)
{
	...
	RTK_CREATE_PROC_READ_ENTRY(p, "debug_statistic", rtl8192cd_proc_debug_statistic);
	...
	//RTK_CREATE_PROC_READ_WRITE_ENTRY(p,"btreg_read", proc_get_btreg_read, proc_set_btreg_read);
	//RTK_CREATE_PROC_READ_WRITE_ENTRY(p,"btreg_write", proc_get_btreg_write, proc_set_btreg_write);
	...
	RTK_CREATE_PROC_READ_WRITE_ENTRY(p, "ccx", rtl8192cd_proc_ccx_read, rtl8192cd_proc_ccx_write);
}
...
//	{"bt_coexist", rtl8192cd_proc_bt_coexist,NULL},
	{"bt_coexist", proc_get_btcoex_info,NULL},		
#endif
#ifdef CONFIG_IEEE80211R
...
#endif
	{"ccx",	rtl8192cd_proc_ccx_read, rtl8192cd_proc_ccx_write};
};
