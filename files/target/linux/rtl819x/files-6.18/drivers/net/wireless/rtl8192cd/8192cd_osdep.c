...
#if defined(CONFIG_RTL_92D_DMDP) || defined(TC_CONCURRENT_FT)
#if defined(NOT_RTK_BSP) && defined(CONFIG_RTL_92D_DMDP)
...
#include <./btcoexist/btcoexist_init.h>
#endif
	...
	if (bcnInt) {
		...
		if(priv->pshare->rf_ft_var.beacon_igi){
			...
			if(GET_CHIP_VER(priv) == VERSION_8822B) {
				...
				RTL_W8(rA_IGI_Jaguar, DM_DIG_MIN_AP);
				RTL_W8(rB_IGI_Jaguar, DM_DIG_MIN_AP);				
			}
#endif
#if defined (CONFIG_WLAN_HAL_8197F) 
			if(GET_CHIP_VER(priv) == VERSION_8197F) {
				...
				RTL_W8(ODM_REG_IGI_A_11N, DM_DIG_MIN_AP);
				RTL_W8(ODM_REG_IGI_B_11N, DM_DIG_MIN_AP);
			}
#endif
		}
#endif
...
)
{
		...
		//GET_HAL_INTERFACE(priv)->RemoveInterruptMaskHandler(priv, HAL_INT_TYPE_PSTIMEOUT);
// for dualband throughput
#if 0	
#if defined(CONFIG_WLAN_HAL_8192EE)
		if (GET_CHIP_VER(priv)==VERSION_8192E) 
			RTL_W32(REG_HIMR0, RTL_R32(REG_HIMR0)  & ~ HIMR_92E_PSTIMEOUT);
		else
#endif
			RTL_W32(REG_HIMR0, RTL_R32(REG_HIMR0)  & ~ BIT_PSTIMEOUT_MSK);
#endif
...
int_retry_process:    
    ...
    // 8.) CPWM2 //yllin
        ...
        //disable 32K
            ...
            //printk("0x284=%x\n",RTL_R32(0x284));
    #ifdef CONFIG_SAPPS
            if(priv->enable_SAPPS){
                FillH2CCmd88XX(priv,0x26,2,H2CCommand);
                printk("send leave SAPPS H2C\n");
            }
    #endif
          while(RTL_R32(0x120) & BIT(16)){
    			if(rxdone_check_count > 20){
    				printk("leave apoffload error, 0x120 BIT16=1\n");
                    return FAIL;
    				//break;
    			}
...
#if defined(MULTI_MAC_CLONE) && defined(UNIVERSAL_REPEATER) && defined(A4_STA) && defined(CONFIG_WLAN_HAL)
	if( IS_HAL_CHIP(priv) && priv->pmib->ethBrExtInfo.macclone_enable && priv->pshare->rf_ft_var.a4_enable) {
		priv->pmib->ethBrExtInfo.macclone_enable =0;
		DEBUG_INFO("Disable macclone due to a4_enable\n");
	}
#endif
	if(GET_CHIP_VER(priv) < VERSION_8812E) {
		priv->pmib->dot11nConfigEntry.dot11nLDPC = 0;
...
do_hw_init:
...
#ifdef ENABLE_BT_COEXIST
		rc = rtl8192cd_bt_coexist_func_init(priv);
#endif
    ...
    if(IS_SUPPORT_HW_FILL_MACID(priv)) {
		...
		if (OPMODE & WIFI_AP_STATE) {
			...
			rtw_btcoex_MediaStatusNotify(priv, 1); /* connect */
		}
#endif /* ENABLE_BT_COEXIST */
...
//		priv->ps_ctrl = 0x11;
		...
		if (priv->pmib->miscEntry.tcpack_acc)
		{
			#ifdef CLIENT_MODE
			if (OPMODE & (WIFI_STATION_STATE | WIFI_ADHOC_STATE))
				priv->pmib->miscEntry.tcpack_use_hw_timer = 0;
			#endif

			#ifdef UNIVERSAL_REPEATER
			if(GET_VXD_PRIV(priv) != NULL)
				priv->pvxd_priv->pmib->miscEntry.tcpack_use_hw_timer = 0;
			#endif
			...
			if (!IEEE8021X_FUN || (IEEE8021X_FUN && (priv->pmib->dot11RsnIE.rsnielen > 0))) {
...
#endif
				{
					...
					if (is_reset){
						...
						if ((OPMODE&WIFI_STATION_STATE) && (OPMODE&WIFI_SITE_MONITOR))
							rtw_btcoex_ScanNotify(priv,0);//+wishchen 
#endif
					}
#endif
...
#ifdef ENABLE_BT_COEXIST
	//init BT coexist hal
	priv->pshare->bt_coex_comm_data.bt_coex_hal_func.hal_init(priv);
#endif
...
int rtl8192cd_close(struct net_device *dev)
{
	...
	if (timer_pending(&priv->ch_avail_chk_timer)) {
		 ...
		 * (8192cd.h). Both are absent in the WEXT build, so this notification —
		 * which only tells the nl80211 stack a DFS CAC was aborted — is skipped.
		 * The register write above (TXPAUSE) is the part that touches hardware and
		 * it is unconditional. */
#ifdef RTK_NL80211
    ...
    STADEBUG("===>\n");
	DRV_RT_TRACE(priv, DRV_DBG_SYS_INFO, DRV_DBG_SERIOUS, "\n");
	SMP_LOCK(flags);
	DBFENTER;
...
*/
...
#endif
					// AP close
					priv->del_sta_info.branch_code = 5;
					set_del_sta_info(priv, priv->pshare->aidarray[i]->station.hwaddr, _RSON_DEAUTH_STA_LEAVING_);
 ...
 && (GET_VXD_PRIV(priv)->simple_config_status == 0)
	...
	DEBUG_INFO("skb_rx_cnt =%d\n", rtl_atomic_read(&priv->rtl_rx_skb_cnt));
	 ...
	 if ((IS_ROOT_INTERFACE(priv)) && (IS_BT_COEXIST_AVALIABLE(priv))){
	  	rtw_btcoex_close_socket(priv);
	 }
	 else{
...
static void MDL_DEVINIT set_mib_default(struct rtl8192cd_priv *priv)
{
	...
	/* R4/G3 Path B: read_flash_hw_cal_data(), read_flash_hw_mac_vap() and
	 * is_WRT_scan_iface() are all DEFINED IN 8192cd_cfg80211.c, and priv->rtk is
	 * declared under #ifdef RTK_NL80211 (8192cd.h).  None of them exist in the WEXT
	 * build.  The compile only warned (implicit-function-declaration is downgraded
	 * for this driver), so without this guard they became modpost undefineds.
	 * The #else leg restores the vendor's own pre-NL80211 default, which is the
	 * commented-out memcpy that used to live right here.
	 * ⚠ FOLLOW-UP: the MAC is therefore the Realtek default, not the board's. The
	 * flash reader those functions used wants an MTD partition named "hwpart",
	 * which does not exist in this OpenWrt layout anyway, so it would have failed
	 * over to exactly this constant. Set the real MAC from userspace
	 * (`ip link set wlanN address ...`) or wire it to the OpenWrt board MAC later. */
#ifdef RTK_NL80211
	if (IS_ROOT_INTERFACE(priv))
...
#else
	name = priv->dev->name;
	(void)name; (void)mac; (void)vap_idx;
	memcpy(p, "\x00\xe0\x4c\x81\x86\x86", MACADDRLEN);
#endif
...
#endif // __KERNEL__
{
	...
	if (!dev) {
		...
		LOG_MSG("alloc_etherdev() error!\n");
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 1;
#ifdef __KERNEL__
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_return_code = -ENOMEM;
		return -ENOMEM;
#else
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_return_code = NULL;
		return NULL;
#endif
	}
	...
	if (!pmib) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 2;
		printk(KERN_ERR "Can't kmalloc for wifi_mib (size %d)\n", sizeof(struct wifi_mib));
		LOG_MSG("Can't kmalloc for wifi_mib (size %d)\n", sizeof(struct wifi_mib));
		goto err_out_free;
	}
	...
	if (!pevent_queue) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 3;
		printk(KERN_ERR "Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		LOG_MSG("Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		goto err_out_free;
	}
	...
	if (!rssim_queue) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 4;
		printk(KERN_ERR "Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		LOG_MSG(KERN_ERR "Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		goto err_out_free;
	}
	...
	if (!wapiEvent_queue) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 5;
		printk(KERN_ERR "Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		LOG_MSG("Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		goto err_out_free;
	}
	...
	if (!pftevent_queue) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 6;
		printk(KERN_ERR "Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		LOG_MSG("Can't kmalloc for DOT11_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
		goto err_out_free;
	}
...
#endif
	{
		...
		if (!phw) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 7;
			printk(KERN_ERR "Can't kmalloc for rtl8192cd_hw (size %d)\n", sizeof(struct rtl8192cd_hw));
			LOG_MSG("Can't kmalloc for rtl8192cd_hw (size %d)\n", sizeof(struct rtl8192cd_hw));
			goto err_out_free;
		}
#else
		...
		if (!dma_virt) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 8;
			printk(KERN_ERR "Can't dma_alloc_coherent for rtl8192cd_hw (size %d)\n", sizeof(struct rtl8192cd_hw));
			LOG_MSG("Can't dma_alloc_coherent for rtl8192cd_hw (size %d)\n", sizeof(struct rtl8192cd_hw));
			goto err_out_free;
		}
		else {
		...
		if (!pshare) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 9;
			printk(KERN_ERR "Can't kmalloc for priv_shared_info (size %d)\n", sizeof(struct priv_shared_info));
			LOG_MSG("Can't kmalloc for priv_shared_info (size %d)\n", sizeof(struct priv_shared_info));
			goto err_out_free;
		}
#endif
		...
		if (!pwlan_hdr_poll) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 10;
			printk(KERN_ERR "Can't kmalloc for wlan_hdr_poll (size %d)\n", sizeof(struct wlan_hdr_poll));
			LOG_MSG("Can't kmalloc for wlan_hdr_poll (size %d)\n", sizeof(struct wlan_hdr_poll));
			goto err_out_free;
		}
#endif
		...
		if (!pwlanllc_hdr_poll) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 11;
			printk(KERN_ERR "Can't kmalloc for wlanllc_hdr_poll (size %d)\n", sizeof(struct wlanllc_hdr_poll));
			LOG_MSG("Can't kmalloc for wlanllc_hdr_poll (size %d)\n", sizeof(struct wlanllc_hdr_poll));
			goto err_out_free;
		}
#endif
		...
		if (!pwlanbuf_poll) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 12;
			printk(KERN_ERR "Can't kmalloc for wlanbuf_poll (size %d)\n", sizeof(struct wlanbuf_poll));
			LOG_MSG("Can't kmalloc for wlanbuf_poll (size %d)\n", sizeof(struct wlanbuf_poll));
			goto err_out_free;
		}
#endif
		...
		if (!pwlanicv_poll) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 13;
			printk(KERN_ERR "Can't kmalloc for wlanicv_poll (size %d)\n", sizeof(struct wlanicv_poll));
			LOG_MSG("Can't kmalloc for wlanicv_poll (size %d)\n", sizeof(struct wlanicv_poll));
			goto err_out_free;
		}
#endif
		...
		if (!pwlanmic_poll) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 14;
			printk(KERN_ERR "Can't kmalloc for wlanmic_poll (size %d)\n", sizeof(struct wlanmic_poll));
			LOG_MSG("Can't kmalloc for wlanmic_poll (size %d)\n", sizeof(struct wlanmic_poll));
			goto err_out_free;
		}
#endif
	}
/*cfg p2p cfg p2p*/	
    ...
    if(p2p_context_ptr == NULL){
        rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 15;
        panic_printk(KERN_ERR "Can't kmalloc for p2p_context (size %d)\n", sizeof(struct p2p_context));
		LOG_MSG("Can't kmalloc for p2p_context (size %d)\n", sizeof(struct p2p_context));
        goto err_out_free;                
    }else{          
	...
	if (!pwlan_acl_poll) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 16;
		printk(KERN_ERR "Can't kmalloc for wlan_acl_poll (size %d)\n", sizeof(struct wlan_acl_poll));
		LOG_MSG("Can't kmalloc for wlan_acl_poll (size %d)\n", sizeof(struct wlan_acl_poll));
		goto err_out_free;
	}
	...
	if (!pmesh_acl_poll) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 17;
		printk(KERN_ERR "Can't kmalloc for Mesh wlan_acl_poll (size %d)\n", sizeof(struct mesh_acl_poll));
		LOG_MSG("Can't kmalloc for Mesh wlan_acl_poll (size %d)\n", sizeof(struct mesh_acl_poll));
		goto err_out_free;
	}
#endif
	...
	if (!Eap_packet) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 18;
		printk(KERN_ERR "Can't kmalloc for Eap_packet (size %d)\n", sizeof(DOT11_EAP_PACKET));
		LOG_MSG("Can't kmalloc for Eap_packet (size %d)\n", sizeof(DOT11_EAP_PACKET));
		goto err_out_free;
	}
	...
	if (!wpa_global_info) {
		rc = -ENOMEM;
		((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 19;
		printk(KERN_ERR "Can't kmalloc for wpa_global_info (size %d)\n", sizeof(WPA_GLOBAL_INFO));
		LOG_MSG("Can't kmalloc for wpa_global_info (size %d)\n", sizeof(WPA_GLOBAL_INFO));
		goto err_out_free;
	}
...
#endif
	{
		...
		if (!site_survey) {
			rc = -ENOMEM;
			((struct rtl8192cd_priv *)netdev_priv(dev))->dev_init_reason_code = 20;
			printk(KERN_ERR "Can't kmalloc for site_survey (size %d)\n", sizeof(struct ss_res));
			LOG_MSG("Can't kmalloc for site_survey (size %d)\n", sizeof(struct ss_res));
			goto err_out_free;
		}
	}
...
register_driver:
...
#endif
	{
		...
		for (i=0; i<wds_num; i++) {
			...
			if (!dev) {
				...
				LOG_MSG("alloc_etherdev() wds error!\n");
				priv->dev_init_reason_code = 21;
				rc = -ENOMEM;
	    	   	goto err_out_dev;
			}
...
#endif
			if (rc) {
				priv->dev_init_reason_code = 22;
				printk(KERN_ERR "register_netdev() wds error!\n");
				LOG_MSG("register_netdev() wds error!\n");
				goto err_out_dev;
			}
#endif
        ...
        if(mesh_num>0) {/*the first wlan interface with mesh enabled*/
...
#endif	//CONFIG_RTL_MESH_SINGLE_IFACE
            {
                ...
                if (!dev) {
					priv->dev_init_reason_code = 23;
                    printk(KERN_ERR "alloc_etherdev() mesh error!\n");
					LOG_MSG("alloc_etherdev() mesh error!\n");
                    rc = -ENOMEM;
                    goto err_out_dev;
                }
                dev->base_addr = 1;
...
#endif
                if (rc) {
					priv->dev_init_reason_code = 24;
                    printk(KERN_ERR "register_netdev() mesh error!\n");
					LOG_MSG("register_netdev() mesh error!\n");
                    goto err_out_dev;
                }
                #endif
            }
        ...
        {
            rc = -ENOMEM;
			priv->dev_init_reason_code = 25;
            printk("allocate pann_mpp_tb error!!\n");
			LOG_MSG("allocate pann_mpp_tb error!!\n");
            goto err_out_dev;
        }
        ...
        {
            rc = -ENOMEM;
			priv->dev_init_reason_code = 27;
            printk("allocate proxyupdate_table error!!\n");
			LOG_MSG("allocate proxyupdate_table error!!\n");
            goto err_out_dev;
        }
        ...
        if (!pathsel_queue) {
            rc = -ENOMEM;
			priv->dev_init_reason_code = 28;
            printk(KERN_ERR "Can't kmalloc for PATHSELECTION_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
			LOG_MSG("Can't kmalloc for PATHSELECTION_QUEUE (size %d)\n", sizeof(DOT11_QUEUE));
            goto err_out_dev;
        }
        ...
        {
            rc = -ENOMEM;
			priv->dev_init_reason_code = 29;
            printk("allocate pathsel_table error!!\n");
			LOG_MSG("allocate pathsel_table error!!\n");
            goto err_out_dev;
        }
        ...
        {
            rc = -ENOMEM;
			priv->dev_init_reason_code = 30;
            printk("allocate mesh_rreq_retry_queue error!!\n");
			LOG_MSG("allocate mesh_rreq_retry_queue error!!\n");
            goto err_out_dev;
        }
        ...
        {
        	priv->dev_init_reason_code = 31;
            printk("init_hash_table \"proxy_table\" error!!\n");
			LOG_MSG("init_hash_table \"proxy_table\" error!!\n");
			goto err_out_free;
        }
        ...
        if(rc == HASH_TABLE_FAILED)
        {
        	priv->dev_init_reason_code = 32;
            printk("init_hash_table \"proxyupdate_table\" error!!\n");
			LOG_MSG("init_hash_table \"proxyupdate_table\" error!!\n");
			goto err_out_free;
        }
#endif
        ...
        if(rc == HASH_TABLE_FAILED)
        {
        	priv->dev_init_reason_code = 33;
            printk("init_hash_table \"pathsel_table\" error!!\n");
			LOG_MSG("init_hash_table \"pathsel_table\" error!!\n");
			goto err_out_free;
        }
        ...
        if(rc == HASH_TABLE_FAILED)
        {
        	priv->dev_init_reason_code = 34;
            printk("init_hash_table \"mesh_rreq_retry_queue\" error!!\n");
			LOG_MSG("init_hash_table \"mesh_rreq_retry_queue\" error!!\n");
			goto err_out_free;
        }
	...
	if(bss11v_tmp_priv == NULL) {
		...
		if (IS_HAL_CHIP(priv)) {
		    ...
		    if (page_ptr == NULL) {
    			rc = -ENOMEM;
				priv->dev_init_reason_code = 35;
    			panic_printk(KERN_ERR "can't allocate descriptior page, abort!\n");
				LOG_MSG("can't allocate descriptior page, abort!\n");
            ...
            if (page_ptr_h2c == NULL) {
                kfree(page_ptr);
                rc = -ENOMEM;
				priv->dev_init_reason_code = 36;
                printk(KERN_ERR "can't allocate h2c buffer, abort!\n");
				LOG_MSG(KERN_ERR "can't allocate h2c buffer, abort!\n");
                goto err_out_dev;
            } else {
                _GET_HAL_DATA(priv)->h2c_buf_len = H2C_BUFFER_SIZE;
            }
            ...
            if (page_ptr_amsdu == NULL) {
                kfree(page_ptr);
                rc = -ENOMEM;
				priv->dev_init_reason_code = 37;
                printk(KERN_ERR "can't allocate AMSDU descriptior page, abort!\n");
				LOG_MSG(KERN_ERR "can't allocate AMSDU descriptior page, abort!\n");
                goto err_out_dev;
            } else {
                _GET_HAL_DATA(priv)->desc_dma_buf_len_amsdu = DESC_DMA_PAGE_SIZE_HAL_FOR_AMSDU;
            }
...
#if defined(__KERNEL__) || defined(__OSK__)
	priv->dev_init_return_code = rc;
    return rc;
#else
	priv->dev_init_return_code = NULL;
	return NULL;
#endif
}
...
#ifdef ENABLE_BT_COEXIST
#ifdef CONFIG_BT_COEXIST_SOCKET_TRX
	if (IS_BT_COEXIST_AVALIABLE(priv)){
		rtw_btcoex_close_socket(priv);
	}
#endif
	rtw_btcoex_HaltNotify(priv);
	rtw_btcoex_MediaStatusNotify(priv, 0); /* disconnect */
...
#if defined(CONFIG_CHECK_TP)
#define CHECK_TP_WIFI
#include "../../rtl819x/check_tp.c"
#undef CHECK_TP_WIFI
#endif
