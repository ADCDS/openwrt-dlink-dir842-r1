...
#include "8192cd_btcoex.h"

#define PADAPTER PRTL8192CD_PRIV
#define PPRIV_SHARED_INFO struct priv_shared_info *
#define PRIV_SHARED_INFO struct priv_shared_info
#define GET_PRIV_SHARED_INFO(x) (x->pshare)
...
{
	PPRIV_SHARED_INFO	pHalData;

	pHalData = GET_PRIV_SHARED_INFO(padapter);
...
void rtw_btcoex_ScanNotify(struct rtl8192cd_priv *priv, u8 type)
{
	PPRIV_SHARED_INFO	pHalData;
#ifdef BT_COEXIST_SOCKET_TRX
	struct bt_coex_info *pcoex_info = &priv->pshare->coex_info;
	PBT_MGNT	pBtMgnt=&pcoex_info->BtMgnt;
#endif //BT_COEXIST_SOCKET_TRX

#if 1 //wishchen 1
	pHalData = GET_PRIV_SHARED_INFO(priv);
	if (_FALSE == pHalData->EEPROMBluetoothCoexist)
		return;
	...
	if(pBtMgnt->ExtConfig.bEnableWifiScanNotify)
		rtw_btcoex_SendScanNotify(priv, type);
#endif //BT_COEXIST_SOCKET_TRX	

	hal_btcoex_ScanNotify(priv, type);
}
...
void rtw_btcoex_BtInfoNotify(struct rtl8192cd_priv *priv, u8 length, u8 *tmpBuf)
{
	PPRIV_SHARED_INFO	pHalData;
	u8 i;
#if 0
...
#endif
	hal_btcoex_BtInfoNotify(priv, length, tmpBuf);
}
...
#endif
void rtw_btcoex_SetBtPatchVersion(struct rtl8192cd_priv *priv,u16 btHciVer, u16 btPatchVer)
{
	hal_btcoex_SetBtPatchVersion(priv,btHciVer,btPatchVer);
}

void rtw_btcoex_SetHciVersion(struct rtl8192cd_priv *priv, u16 hciVersion)
{
	hal_btcoex_SetHciVersion(priv, hciVersion);
}
...
void rtw_btcoex_PTAOffOnNotify(struct rtl8192cd_priv *priv, u8 bBTON)
{
	hal_btcoex_PTAOffOnNotify(priv, bBTON);
}
...
struct rtl8192cd_priv *pbtcoexpriv = NULL;
...
/* modified wishchen*/
u8 rtw_btcoex_btinfo_cmd(struct rtl8192cd_priv *priv, u8 *buf, u16 buf_len)
{
...
#define BTINFO_BT_AUTO_RPT 0x27
	...
	rtw_btcoex_BtInfoNotify(priv,len+1,&buf[1]); //+wishchen
...
#endif
u8 rtw_btcoex_send_event_to_BT(struct rtl8192cd_priv *priv, u8 status,  u8 event_code, u8 opcode_low, u8 opcode_high,u8 *dbg_msg)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
...
#endif		
	status = rtw_btcoex_sendmsgbysocket(priv, (u8 *)pEvent, tx_event_length, _FALSE);
...
u8 rtw_btcoex_parse_BT_info_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
...
#endif
	{
		if(0x1 == btInfoReason || 0x2 == btInfoReason)
		{
			...
			rtw_btcoex_btinfo_cmd(priv,btinfo,btInfoLen);
		}
		else
		{
...
u8 rtw_btcoex_parse_BT_patch_ver_info_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS status=HCI_STATUS_SUCCESS;
	...
	rtw_btcoex_SetBtPatchVersion(priv,btHciVer,btPatchVer);
...
u8 rtw_btcoex_parse_HCI_Ver_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS status=HCI_STATUS_SUCCESS;
	...
	{
		rtw_btcoex_SetHciVersion(priv,hciver);
	}
	//send complete event to BT
	{
		pEvent = (rtw_HCI_event *)(&localBuf[0]);
		...
		//total tx event length + EventCode length + sizeof(length)
		tx_event_length = pEvent->Length + 2;
		panic_printk("Data[1]=%u, Data[2]=%u",pEvent->Data[1],pEvent->Data[2]);
		status = rtw_btcoex_sendmsgbysocket(priv, (u8 *)pEvent, tx_event_length, _FALSE);  //wishchen-
//		status = rtw_btcoex_sendmsgbysocket(priv, "This Works!", tx_event_length, _FALSE);//wishchen+
		return status;
		//bthci_IndicateEvent(Adapter, PPacketIrpEvent, len+2);
	}
...
u8 rtw_btcoex_parse_WIFI_scan_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS status=HCI_STATUS_SUCCESS;
...
u8 rtw_btcoex_parse_HCI_link_status_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	RTW_HCI_STATUS	status=HCI_STATUS_SUCCESS;
	struct bt_coex_info	*pcoex_info=&priv->pshare->coex_info;
	PBT_MGNT	pBtMgnt=&pcoex_info->BtMgnt;
...
u8 rtw_btcoex_parse_HCI_BT_coex_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len=0, tx_event_length =0;
...
u8 rtw_btcoex_parse_HCI_BT_operation_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len=0, tx_event_length =0;
...
u8 rtw_btcoex_parse_BT_AFH_MAP_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len=0, tx_event_length =0;
...
u8 rtw_btcoex_parse_BT_register_val_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
...
u8 rtw_btcoex_parse_HCI_BT_abnormal_notify_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len=0, tx_event_length =0;
...
u8 rtw_btcoex_parse_HCI_query_RF_status_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 cmdlen)
{
	u8 localBuf[6] = "";
	u8 *pRetPar;
	u8	len=0, tx_event_length =0;
...
//	 |	OCF			             |	   OGF       |
void rtw_btcoex_parse_hci_extend_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 len,const u16 hci_OCF)
{
	...
	switch(hci_OCF)
	{
		case HCI_EXTENSION_VERSION_NOTIFY:
			...
			rtw_btcoex_parse_HCI_Ver_notify_cmd(priv,pcmd, len);
			break;
		case HCI_LINK_STATUS_NOTIFY:
			DBG_871X("HCI_LINK_STATUS_NOTIFY\n");
			rtw_btcoex_parse_HCI_link_status_notify_cmd(priv,pcmd, len);
			break;
		case HCI_BT_OPERATION_NOTIFY:
			// only for 8723a 2ant
			DBG_871X("HCI_BT_OPERATION_NOTIFY\n");
			rtw_btcoex_parse_HCI_BT_operation_notify_cmd(priv,pcmd, len);
			//
			break;
		case HCI_ENABLE_WIFI_SCAN_NOTIFY:
			DBG_871X("HCI_ENABLE_WIFI_SCAN_NOTIFY\n");
			rtw_btcoex_parse_WIFI_scan_notify_cmd(priv,pcmd, len);
			break;
		case HCI_QUERY_RF_STATUS:
			// only for 8723b 2ant
			DBG_871X("HCI_QUERY_RF_STATUS\n");
			rtw_btcoex_parse_HCI_query_RF_status_cmd(priv,pcmd, len);
			break;
		case HCI_BT_ABNORMAL_NOTIFY:
			DBG_871X("HCI_BT_ABNORMAL_NOTIFY\n");
			rtw_btcoex_parse_HCI_BT_abnormal_notify_cmd(priv,pcmd, len);
			break;
		case HCI_BT_INFO_NOTIFY:
			DBG_871X("HCI_BT_INFO_NOTIFY\n");
			rtw_btcoex_parse_BT_info_notify_cmd(priv,pcmd, len);
			break;
		case HCI_BT_COEX_NOTIFY:
			DBG_871X("HCI_BT_COEX_NOTIFY\n");
			rtw_btcoex_parse_HCI_BT_coex_notify_cmd(priv,pcmd, len);
			break;
		case HCI_BT_PATCH_VERSION_NOTIFY:
			DBG_871X("HCI_BT_PATCH_VERSION_NOTIFY\n");
			rtw_btcoex_parse_BT_patch_ver_info_cmd(priv,pcmd, len);
			break;
		case HCI_BT_AFH_MAP_NOTIFY:
			DBG_871X("HCI_BT_AFH_MAP_NOTIFY\n");
			rtw_btcoex_parse_BT_AFH_MAP_notify_cmd(priv,pcmd, len);
			break;
		case HCI_BT_REGISTER_VALUE_NOTIFY:
			DBG_871X("HCI_BT_REGISTER_VALUE_NOTIFY\n");
			rtw_btcoex_parse_BT_register_val_notify_cmd(priv,pcmd, len);
			break;
		default:
...
void rtw_btcoex_parse_hci_cmd(struct rtl8192cd_priv *priv, u8 *pcmd, u16 len)
{
	...
	switch(hci_OGF)
	{
		case OGF_EXTENSION:
			...
			rtw_btcoex_parse_hci_extend_cmd(priv, &pcmd[3], cmdlen, hci_OCF);
			break;
		default:
...
u16 rtw_btcoex_parse_recv_data(u8 *msg, u8 msg_size)
{
	...
	if (memcmp(cmp_msg1, msg, msg_size) == 0) {
		DBG_871X("Got attend_ackmsg, msg:%s\n",msg);
		res = RX_ATTEND_ACK;
	} else if (memcmp(cmp_msg2, msg, msg_size) == 0) {
		DBG_871X("Got leave_ack, msg:%s\n",msg);
		res = RX_LEAVE_ACK;
	} else if (memcmp(cmp_msg3, msg, msg_size) == 0) {
		DBG_871X("Got bt_leave, msg:%s\n",msg);
		res = RX_BT_LEAVE;
	} else if (memcmp(cmp_msg4, msg, msg_size) == 0) {
		DBG_871X("Got invite_req, msg:%s\n",msg);
		res = RX_INVITE_REQ;
	} else if (memcmp(cmp_msg5, msg, msg_size) == 0) {
		DBG_871X("Got attend_req, msg:%s\n",msg);
		res = RX_ATTEND_REQ;
	} else if (memcmp(cmp_msg6, msg, msg_size) == 0) {
		DBG_871X("Got invite_rsp, msg:%s\n",msg);
		res = RX_INVITE_RSP;
	} else {
		DBG_871X("Got Others, msg:%s\n", msg);
		res = OTHER;
	}
...
void rtw_btcoex_recvmsgbysocket(void *data)
{
	...
	pcoex_info = &pbtcoexpriv->pshare->coex_info;
...
/*
		if (RX_ATTEND_ACK == parse_res) {
			...
			rtw_btcoex_sendmsgbysocket(pbtcoexpriv, attend_ack, sizeof(attend_ack), _FALSE);
		} else if (RX_INVITE_REQ == parse_res) {
			//invite req from BT
			pcoex_info->BT_attend = _TRUE;
			DBG_871X("RX_INVITE_REQ!,sock_open:%d, BT_attend:%d\n", pcoex_info->sock_open, pcoex_info->BT_attend);
			rtw_btcoex_sendmsgbysocket(pbtcoexpriv, invite_rsp, sizeof(invite_rsp), _FALSE);
			...
			//BT leave
			rtw_btcoex_sendmsgbysocket(pbtcoexpriv, leave_ack, sizeof(leave_ack), _FALSE); // no ack
			pcoex_info->BT_attend = _FALSE;
			DBG_871X("RX_BT_LEAVE!sock_open:%d, BT_attend:%d\n", pcoex_info->sock_open, pcoex_info->BT_attend);			
		} else {
			//todo: check if recv data are really hci cmds
			if (_TRUE == pcoex_info->BT_attend)
				rtw_btcoex_parse_hci_cmd(pbtcoexpriv, recv_data, recv_length);
		}
*/
		switch (parse_res) {
		case RX_ATTEND_ACK:
			...
			pcoex_info->BT_attend = _TRUE;
			DBG_871X("RX_ATTEND_ACK!,sock_open:%d, BT_attend:%d\n", pcoex_info->sock_open, pcoex_info->BT_attend);
			rtw_btcoex_PTAOffOnNotify(pbtcoexpriv, pcoex_info->BT_attend);
			break;
...
u8 rtw_btcoex_create_kernel_socket(struct rtl8192cd_priv *priv)
{
	...
	if (kernel_socket_err < 0) { 
		...
		if (kernel_socket_err == 0) {	
			...
			rtw_btcoex_sendmsgbysocket(priv, attend_req, sizeof(attend_req), _TRUE);
			status = _SUCCESS;
		} else { 
			pcoex_info->BT_attend = _FALSE;
...
void rtw_btcoex_close_kernel_socket(struct rtl8192cd_priv *priv)
{
	struct bt_coex_info *pcoex_info = &priv->pshare->coex_info;
...
void rtw_btcoex_init_socket(struct rtl8192cd_priv *priv)
{
	...
	if (_FALSE == pcoex_info->is_exist) {
		...
		INIT_DELAYED_WORK(&pcoex_info->recvmsg_work, (void *)rtw_btcoex_recvmsgbysocket);
		pbtcoexpriv = priv;
		/* We expect BT is off if BT don't send ack to wifi */
		DBG_871X("We expect BT is off if BT send ack to wifi\n");
		rtw_btcoex_PTAOffOnNotify(pbtcoexpriv, _FALSE);
	#ifdef CONFIG_PCI_HCI
		rtw_btcoex_PTAOffOnNotify(pbtcoexpriv, _TRUE);
	#endif
		if (rtw_btcoex_create_kernel_socket(priv) == _SUCCESS) {
		...
		DBG_871X("%s: pbtcoexpriv:%p, coex_info->is_exist: %s\n"
			, __func__, pbtcoexpriv, pcoex_info->is_exist == _TRUE?"TRUE":"FALSE");
	}
}

void rtw_btcoex_close_socket(struct rtl8192cd_priv *priv)
{
	struct bt_coex_info *pcoex_info = &priv->pshare->coex_info;
	...
	if (_TRUE == pcoex_info->is_exist) {
		if (_TRUE == pcoex_info->BT_attend) {
			...
			rtw_btcoex_sendmsgbysocket(priv, wifi_leave, sizeof(wifi_leave), _FALSE);
			msleep(50);
		}
		...
		rtw_btcoex_close_kernel_socket(priv);
		pbtcoexpriv = NULL;
		pcoex_info->is_exist = _FALSE;
	}
}
...
void rtw_btcoex_SendEventExtBtCoexControl(struct rtl8192cd_priv *priv, u8 bNeedDbgRsp, u8 dataLen, void *pData)
{
	u8			len=0, tx_event_length = 0;
	u8 			localBuf[32] = "";
	u8			*pRetPar;
...
#endif
	rtw_btcoex_sendmsgbysocket(priv, (u8 *)pEvent, tx_event_length, _FALSE);

}

/* Porting from Windows team */
void rtw_btcoex_SendEventExtBtInfoControl(struct rtl8192cd_priv *priv, u8 dataLen, void *pData)
{
	rtw_HCI_event *pEvent;
	u8			*pRetPar;
	u8			len=0, tx_event_length = 0;
...
void rtw_btcoex_SendScanNotify(struct rtl8192cd_priv *priv, u8 scanType)
{
	u8	len=0, tx_event_length=0;
	u8 	localBuf[7] = "";
	u8	*pRetPar;
