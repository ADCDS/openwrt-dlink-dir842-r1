...
void phydm_dig_recorder(void *dm_void, boolean first_connect, u8 igi_curr, u32 fa_cnt)
{
	...
	PHYDM_DBG(dm, ODM_COMP_DIG, "igi_history[3:0] = {0x%x, 0x%x, 0x%x, 0x%x}\n",
		  dig_rc->igi_history[3], dig_rc->igi_history[2],
		  dig_rc->igi_history[1], dig_rc->igi_history[0]);
	PHYDM_DBG(dm, ODM_COMP_DIG, "fa_history[3:0] = {%d, %d, %d, %d}\n",
		  dig_rc->fa_history[3], dig_rc->fa_history[2],
		  dig_rc->fa_history[1], dig_rc->fa_history[0]);
	PHYDM_DBG(dm, ODM_COMP_DIG, "igi_bitmap = {%d, %d, %d, %d} = 0x%x\n",
		  (dig_rc->igi_bitmap & BIT(3)) >> 3,
		  (dig_rc->igi_bitmap & BIT(2)) >> 2,
		  (dig_rc->igi_bitmap & BIT(1)) >> 1,
		  dig_rc->igi_bitmap & BIT(0),
		  dig_rc->igi_bitmap);
}
...
void phydm_dig_damping_chk(void *dm_void)
{
	...
	if (dig_rc->damping_limit_en) {
		...
		if (DIFF_2(dm->RSSI_Min, dig_rc->limit_rssi) > 3 ||
			...
			PHYDM_DBG(dm, ODM_COMP_DIG, "rssi_min=%d, limit_rssi=%d\n",
	...
	PHYDM_DBG(dm, ODM_COMP_DIG, "fa_th{H, L}= {%d,%d}\n", fa_high_th, fa_low_th);
	...
	PHYDM_DBG(dm, ODM_COMP_DIG, "damping_limit=%d\n", dig_rc->damping_limit_en);
}
#endif
...
#endif
#ifdef CFG_DIG_DAMPING_CHK
	phydm_dig_recorder_reset(pDM_Odm);
	pDM_DigTable->dig_dl_en = 1;
#endif
}
...
//	PFALSE_ALARM_STATISTICS		pFalseAlmCnt = (PFALSE_ALARM_STATISTICS)PhyDM_Get_Structure( pDM_Odm, PHYDM_FALSEALMCNT);
...
#ifdef CFG_DIG_DAMPING_CHK
	/*Record IGI History*/
	phydm_dig_recorder(pDM_Odm, FirstConnect, CurrentIGI, pFalseAlmCnt->Cnt_all);

	/*DIG Damping Check*/
	phydm_dig_damping_chk(pDM_Odm);
#endif
	...
	if (pDM_DigTable->dig_dl_en &&
	    dig_rc->damping_limit_en &&
	    pDM_DigTable->rx_gain_range_min < dig_rc->damping_limit_val) {
		PHYDM_DBG(pDM_Odm, ODM_COMP_DIG,
	...
	pDM_DigTable->fa_th[0] = dm_FA_thres[0];
	pDM_DigTable->fa_th[1] = dm_FA_thres[1];
	pDM_DigTable->fa_th[2] = dm_FA_thres[2];
	...
	)
{
	...
	if(pDM_Odm->bLinked && bPerformance)
	{
		...
		if((igi_upper_rssi_min) > dm_dig_max)
			pDM_DigTable->tdma_rx_gain_max[TDMA_DIG_HIGH_STATE] = dm_dig_max;
		else if((igi_upper_rssi_min) < dm_dig_min)
...
{
	PDM_ODM_T				pDM_Odm = (PDM_ODM_T)pDM_VOID;	
	PLNA_SAT_TYPE2_INFO		pinfo = &pDM_Odm->DM_LNA_SAT_TYPE2_INFO;
//	u1Byte					real_shift = (pDM_Odm->total_bit_shift!=0)? (pDM_Odm->total_bit_shift): SNR_STATISTIC_SHIFT;

	u1Byte					real_shift = pDM_Odm->total_bit_shift;

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("%s ==>\n", __FUNCTION__));

	pinfo->total_cnt_snr_statistic = 1 << real_shift;
...
//		pinfo->sum_snr_statistic += rx_snr;
		...
		if(pinfo->cnt_snr_statistic == (pinfo->total_cnt_snr_statistic - 1)) {
...
//	u1Byte	real_shift = (pDM_Odm->total_bit_shift!=0)? (pDM_Odm->total_bit_shift): SNR_STATISTIC_SHIFT;
	u1Byte	real_shift = pDM_Odm->total_bit_shift;
	u2Byte	total_snr_cnt = pinfo->total_cnt_snr_statistic;
	u2Byte	total_loop_cnt = (total_snr_cnt - 1), i;
	u4Byte	temp;
	u4Byte	sum_snr_statistic = 0;

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("%s ==>\n", __FUNCTION__));		
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("total_loop_cnt=%d\n", total_loop_cnt));

	for(i = 0; (i <= total_loop_cnt); i++) {

		if (pDM_Odm->is_snr_detail_en) {
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("snr[%d]=%d\n", i, pinfo->snr_statistic[i]));
		}

		sum_snr_statistic += pinfo->snr_statistic[i];
		
		pinfo->snr_statistic_sqr[i] =
			pinfo->snr_statistic[i] * pinfo->snr_statistic[i];
	}

	phydm_bubble_sort(pDM_Odm, pinfo->snr_statistic, pinfo->total_cnt_snr_statistic);
...
/*							
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, 
		("snr_higher_cnt=%d, snr_lower_cnt=%d\n", snr_higher_cnt, snr_lower_cnt));
*/							
	//ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("pinfo->cur_snr_var\n", pinfo->cur_snr_var));
	...
	temp = 0;
	pinfo->cnt_lower_snr_statistic = (total_snr_cnt >> pDM_Odm->lwr_snr_ratio_bit_shift);	
	for(i = 0; i <= pinfo->cnt_lower_snr_statistic; i++) {
		temp += pinfo->snr_statistic[i];
	}
	
	pinfo->cur_lower_snr_mean = temp >> (real_shift - pDM_Odm->lwr_snr_ratio_bit_shift);

	/*update cnt_lower_snr_statistic*/

}


bool
phydm_is_snr_improve(
	IN		PVOID		pDM_VOID
	)
{
	PDM_ODM_T			pDM_Odm = (PDM_ODM_T)pDM_VOID;
	PLNA_SAT_TYPE2_INFO	pinfo = &pDM_Odm->DM_LNA_SAT_TYPE2_INFO;
//	bool				is_mean_improve = FALSE, is_var_improve = FALSE;
	bool				is_snr_improve;
	u1Byte				cur_state = pinfo->nxt_state;
	u4Byte				cur_mean = pinfo->cur_snr_mean;
	u4Byte				pre_mean = pinfo->pre_snr_mean;
	u4Byte				cur_lower_mean = pinfo->cur_lower_snr_mean;
	u4Byte				pre_lower_mean = pinfo->pre_lower_snr_mean;
	u4Byte				cur_var = pinfo->cur_snr_var;
//	u4Byte				pre_var = pinfo->pre_snr_var;
//	u4Byte				diff_var=0, diff_mean=0;

	/*special case, zero VAR, interference is gone*/
	if ((cur_state == SAT_TABLE_MONITOR) || (cur_state == ORI_TABLE_TRAINING)) {	/*make sure pre_var is larger enough*/
...
/*	
	if (cur_state == ORI_TABLE_MONITOR) {	//make sure pre_var is larger enough
		if (cur_mean < pre_mean) {
			if (cur_var < pre_var) {
...
*/
	...
	if (cur_lower_mean >= (pre_lower_mean + pDM_Odm->delta_snr_mean)) {
		is_snr_improve = TRUE;
	} else {
...
/*	
	if (cur_mean > (pre_mean + pDM_Odm->delta_snr_mean)) {
		is_mean_improve = TRUE;
		is_var_improve = (cur_var <= pre_var + pDM_Odm->delta_snr_var) ? TRUE : FALSE;
		
	} else if (cur_var + pDM_Odm->delta_snr_var <= pre_var) {
		is_var_improve = TRUE;
	...
	)
{
	PDM_ODM_T			pDM_Odm = (PDM_ODM_T)pDM_VOID;
	PLNA_SAT_TYPE2_INFO	pinfo = &pDM_Odm->DM_LNA_SAT_TYPE2_INFO;
//	bool				is_mean_dgrade = FALSE, is_var_degrade = FALSE;
	u4Byte				cur_lower_mean = pinfo->cur_lower_snr_mean;
	u4Byte				pre_lower_mean = pinfo->pre_lower_snr_mean;
	bool				is_degrade;
...
/*
	is_mean_dgrade = (pinfo->cur_snr_mean + pDM_Odm->delta_snr_mean <= pinfo->pre_snr_mean) ? TRUE : FALSE;
	is_var_degrade = (pinfo->cur_snr_var > (pinfo->pre_snr_var + pDM_Odm->delta_snr_mean)) ? TRUE : FALSE;

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("%s: cur_mean=%d, pre_mean=%d, cur_var=%d, pre_var=%d\n",
		__FUNCTION__,
		pinfo->cur_snr_mean,
		pinfo->pre_snr_mean,
		pinfo->cur_snr_var,
	...
	)
{
	PDM_ODM_T			pDM_Odm = (PDM_ODM_T)pDM_VOID;
	PLNA_SAT_TYPE2_INFO	pinfo = &pDM_Odm->DM_LNA_SAT_TYPE2_INFO;
	bool				is_large_var = (pinfo->cur_snr_var >= pDM_Odm->snr_var_thd) ? TRUE : FALSE;
	
	return is_large_var;
}

void
phydm_update_pre_status(
	IN		PVOID		pDM_VOID
	)
{
	PDM_ODM_T				pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	)
{
	...
	if (phydm_is_large_var(pDM_Odm)) {
		pinfo->nxt_state = SAT_TABLE_TRAINING;
		config_phydm_switch_agc_tab_8822b(pDM_Odm, *pDM_Odm->pChannel, LNA_SAT_AGC_TABLE);
	}
	else {
		pinfo->nxt_state = ORI_TABLE_MONITOR;
		/*switch to anti-sat table*/
		config_phydm_switch_agc_tab_8822b(pDM_Odm, *pDM_Odm->pChannel, DEFAULT_AGC_TABLE);
	}
	phydm_update_pre_status(pDM_Odm);	
	pinfo->pre_state = ORI_TABLE_MONITOR;

}

void
phydm_sat_table_training(
	IN		PVOID		pDM_VOID
	)
{
	PDM_ODM_T				pDM_Odm = (PDM_ODM_T)pDM_VOID;
...
/*
	if (pinfo->pre_state == ORI_TABLE_MONITOR) {
		pinfo->nxt_state = SAT_TABLE_TRAINING;
		config_phydm_switch_agc_tab_8822b(pDM_Odm, *pDM_Odm->pChannel, LNA_SAT_AGC_TABLE);
		//ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("%s bypass update pre status statistics once.\n", __FUNCTION__));	
	} else {
	*/
		if (phydm_is_snr_improve(pDM_Odm)) {
	...
	)
{
	...
	if (phydm_is_large_var(pDM_Odm)) {
		if (phydm_is_snr_degrade(pDM_Odm)) {
			pinfo->nxt_state = SAT_TABLE_TRAINING;
			config_phydm_switch_agc_tab_8822b(pDM_Odm, *pDM_Odm->pChannel, LNA_SAT_AGC_TABLE);
		}
		else {
	...
	)
{
	...
	if (phydm_is_snr_improve(pDM_Odm)) {
		...
		if (pinfo->sat_table_monitor_times == pDM_Odm->force_change_period) {
			
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD,
			("%s: sat_table_monitor_times=%d\n", __FUNCTION__, pinfo->sat_table_monitor_times));
	...
	)
{
	...
	if (phydm_is_snr_degrade(pDM_Odm) == FALSE) {
		pinfo->nxt_state = ORI_TABLE_MONITOR;
	} else {
	...
	)
{
	...
	// if pre_state = ORI_TABLE_TRAINING, "pre" adapt ori-table, "cur" adapt sat-table	
	// if pre_state = ORI_TABLE_TRY_FAIL, "pre" adapt sat-table, "cur" adapt sat-table
	...
	if (pinfo->pre_state == ORI_TABLE_TRY_FAIL) {
		if (phydm_is_snr_improve(pDM_Odm)) {
			...
			if (pinfo->ori_table_try_fail_times == pDM_Odm->force_change_period) {

				ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD,
				("%s: ori_table_try_fail_times=%d\n",__FUNCTION__, pinfo->ori_table_try_fail_times));
...
//	pDIG_T						pDM_DigTable = &pDM_Odm->DM_DigTable;
	PLNA_SAT_TYPE2_INFO			pinfo = &pDM_Odm->DM_LNA_SAT_TYPE2_INFO;
	u1Byte						state = pinfo->nxt_state;
	u1Byte						agc_tab = ODM_GetBBReg(pDM_Odm, 0x958, 0x1f);
	...
	if ((pDM_Odm->SupportICType & ODM_RTL8822B) == FALSE) {
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("ODM_BB_LNA_SAT_CHK_TYPE2 only support 22B.\n"));
		return;
	}

	if((pDM_Odm->SupportAbility & ODM_BB_LNA_SAT_CHK_TYPE2) == FALSE) {
		phydm_lna_sat_chk_type2_init(pDM_Odm);
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("ODM_BB_LNA_SAT_CHK_TYPE2 is NOT supported, cur table=%d\n",agc_tab));		
		return;
	}

	if (pinfo->is_snr_done)
		phydm_snr_data_processing(pDM_Odm);
	else
		return;

	ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("cur agc table %d\n", agc_tab));
	...
	if (pDM_Odm->is_force_lna_sat_table != AUTO_AGC_TABLE) {
		...
		if (pinfo->is_snr_done) {
			...
			if (pDM_Odm->is_force_lna_sat_table == DEFAULT_AGC_TABLE)
				config_phydm_switch_agc_tab_8822b(pDM_Odm, *pDM_Odm->pChannel, DEFAULT_AGC_TABLE);
			else if (pDM_Odm->is_force_lna_sat_table == LNA_SAT_AGC_TABLE)
				config_phydm_switch_agc_tab_8822b(pDM_Odm, *pDM_Odm->pChannel, LNA_SAT_AGC_TABLE);
			else
				config_phydm_switch_agc_tab_8822b(pDM_Odm, *pDM_Odm->pChannel, DEFAULT_AGC_TABLE);
		...
		switch(state) {
			...
			case	ORI_TABLE_MONITOR:
				dbg_message = "ORI_TABLE_MONITOR";
				phydm_ori_table_monitor(pDM_Odm);
				break;

			case	SAT_TABLE_TRAINING:
				dbg_message = "SAT_TABLE_TRAINING";
				phydm_sat_table_training(pDM_Odm);
				break;

			case	SAT_TABLE_TRY_FAIL:
				dbg_message = "SAT_TABLE_TRY_FAIL";
				phydm_sat_table_try_fail(pDM_Odm);
				break;

			case	SAT_TABLE_MONITOR:
				dbg_message = "SAT_TABLE_MONITOR";
				phydm_sat_table_monitor(pDM_Odm);
				break;

			case	ORI_TABLE_TRAINING:
				dbg_message = "ORI_TABLE_TRAINING";
				phydm_ori_table_training(pDM_Odm);
				break;

			case 	ORI_TABLE_TRY_FAIL:
				dbg_message = "ORI_TABLE_TRAINING";
				phydm_ori_table_try_fail(pDM_Odm);
				break;
		...
		dbg_message = phydm_lna_sat_state_msg(pDM_Odm, state);
		nxt_dbg_message = phydm_lna_sat_state_msg(pDM_Odm, pinfo->nxt_state);
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_LNA_SAT_CHK, ODM_DBG_LOUD, ("state: [%s]->[%s], \n",	dbg_message, nxt_dbg_message));
