...
)
{
	...
	} else if (dm_value[0] == 6) { /*soml_ratio_th for SOML*/

		p_dm_soml_table->soml_ratio_th = (u8)dm_value[1];
		PHYDM_SNPRINTF((output + used, out_len - used, "soml_ratio_th = ((%d))\n", p_dm_soml_table->soml_ratio_th));
	} else if (dm_value[0] == 7) { /*soml_diff_th for SOML*/

		p_dm_soml_table->soml_diff_th= (u8)dm_value[1];
		PHYDM_SNPRINTF((output + used, out_len - used, "soml_diff_th = ((%d))\n", p_dm_soml_table->soml_diff_th));
...
)
{
	...
	u32	rate_on = 0, rate_off = 0, rate_ratio = 0, rate_diff = 0;
	u32	ht_reset[HT_RATE_IDX] = {0}, vht_reset[VHT_RATE_IDX] = {0};
	u8	size = sizeof(ht_reset[0]);
	...
	if ((p_dm_odm->SupportICType & ODM_ADAPTIVE_SOML_SUPPORT_IC)) {
		if (p_dm_odm->number_active_client == 1) {
...
#endif
				/* [Decision] */
				/*@ 8 bit/(150 ms) ;  <<3(8bit), <<10(10^-3,ms) >>20(10^6,M) */
				rate_on = (byte_total_on >> 7) / (p_dm_soml_table->soml_intvl * (p_dm_soml_table->soml_train_num >> 1));
				rate_off = (byte_total_off >> 7) / (p_dm_soml_table->soml_intvl * (p_dm_soml_table->soml_train_num >> 1));
				if ((rate_on != 0) && (rate_off != 0)) {
					rate_diff = (rate_on > rate_off) ? (rate_on - rate_off) : (rate_off - rate_on);
					rate_ratio = (rate_on > rate_off) ? ((rate_diff * 100) / rate_on) : ((rate_diff * 100) / rate_off);
					if ((rate_diff > p_dm_soml_table->soml_diff_th) && (rate_ratio > p_dm_soml_table->soml_ratio_th))
						p_dm_soml_table->soml_mode = SOML_STOP;
					else
						p_dm_soml_table->soml_mode = SOML_NORMAL;
					ODM_RT_TRACE(p_dm_odm, ODM_COMP_ADAPTIVE_SOML, ODM_DBG_LOUD, ("[ soml_diff_th = %d ; soml_ratio_th = %d ]\n", p_dm_soml_table->soml_diff_th, p_dm_soml_table->soml_ratio_th));
					ODM_RT_TRACE(p_dm_odm, ODM_COMP_ADAPTIVE_SOML, ODM_DBG_LOUD, ("[ rate_on = %d Mbps; rate_off = %d Mbps; rate_diff = %d Mbps; rate_ratio = %d]\n", rate_on, rate_off, rate_diff, rate_ratio));
			...
			if (p_dm_odm->SupportICType==ODM_RTL8197F && p_dm_odm->priv->pshare->tc_msta_dual)
...
)
{
	...
	p_dm_soml_table->soml_ratio_th = 20;
	p_dm_soml_table->soml_diff_th = 50;
	p_dm_soml_table->soml_stop_cnt = 60;
	p_dm_soml_table->soml_mode = SOML_NORMAL;
...
)
{
	...
	if (p_dm_soml_table->soml_mode == SOML_STOP) {
		if (p_dm_soml_table->soml_counter <  p_dm_soml_table->soml_stop_cnt) {
			p_dm_soml_table->soml_counter++;
			return;
		} else
			p_dm_soml_table->soml_counter = 0;
	} else if (p_dm_soml_table->soml_mode == SOML_NORMAL) {
