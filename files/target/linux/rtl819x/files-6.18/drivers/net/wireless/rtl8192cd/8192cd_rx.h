				...
				struct stat_info *pstat, struct rx_frinfo *pfrinfo)
{
	...
	if (pfrinfo->physt) {
			...
			u1Byte	prefer_lna_sat_macid = (ODMPTR->is_force_lna_sat_macid == 0) ? priv->pshare->macid_rssi_min : ODMPTR->is_force_lna_sat_macid;
			bool	is_support = (ODMPTR->SupportAbility & ODM_BB_LNA_SAT_CHK_TYPE2) ? TRUE : FALSE;
				
				if ((pstat->aid == prefer_lna_sat_macid) && (is_support == TRUE)) {
					phydm_snr_collect(ODMPTR, pstat->rx_snr[0]);
				}
#endif
