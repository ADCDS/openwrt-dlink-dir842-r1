...
#define TX_NO_TCPACK_ACC		6//TCP_ACK_ACC
...
#endif
{
	...
	if (parsing) {
		...
		if((OPMODE & WIFI_AP_STATE) && (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_2G) 
			&& priv->pshare->tc_msta_mode
		){
			if(pstat && (pstat->tx_ra_bitmap & 0xff00000))
				pri = 7; //VO
			else
				pri = 1; //BK
		}	
...
void amsdu_timeout(struct rtl8192cd_priv *priv, unsigned int current_time)
{
	...
	while (CIRC_CNT(head, priv->pshare->amsdu_timer_tail, AMSDU_TIMER_NUM)) {
		...
		if (TSF_LESS(current_time, priv->pshare->amsdu_timer[priv->pshare->amsdu_timer_tail].timeout))
			break;
	...
	*bNProtect = FALSE;
	...
	if ((txcfg->rts_thrshld <= get_mpdu_len(txcfg, fr_len)) ||
		...
		*bRtsEnable = TRUE;
		...
		priv->rts_enable_reason = (RTS_RC_RTSEN | RTS_RC_OUT_OF_RTS_THRESHOLD);
#endif
	} else {
		if ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11N) &&
			...
			*bNProtect = 1;
			...
			if (priv->pshare->total_assoc_num > 10)	{
				...
				*bRtsEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
				priv->rts_enable_reason = (RTS_RC_RTSEN | RTS_RC_HT_PROTECTION);
#endif
			} else
#endif
...
#ifdef _DEBUG_RTL8192CD_
				priv->rts_enable_reason = (RTS_RC_CTS2SELF | RTS_RC_HT_PROTECTION);
#endif
				} else {
					*bRtsEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
			...
			*bErpProtect = 1;
			if (priv->pmib->dot11ErpInfo.ctsToSelf) {
				*bCts2SelfEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
				priv->rts_enable_reason = (RTS_RC_CTS2SELF | RTS_RC_ERP_PROTECTION);
#endif
			} else {
				*bRtsEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
				priv->rts_enable_reason = (RTS_RC_RTSEN | RTS_RC_ERP_PROTECTION);
#endif
			}
			...
			*bRtsEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
			priv->rts_enable_reason = (RTS_RC_RTSEN | RTS_RC_HT_MIMO_PS_DYNAMIC);
#endif
			...
			if (is_MCS_rate(txcfg->tx_rate) && (txcfg->pstat->IOTPeer == HT_IOT_PEER_INTEL) && priv->pshare->rf_ft_var.rts_iot_th) {
				...
				*bNProtect = 1;
				if (!txcfg->pstat->no_rts) {
					...
					*bRtsEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
					priv->rts_enable_reason = (RTS_RC_HWRTSEN | RTS_RC_RTSEN | RTS_RC_HW_CTRL);
#endif
				} else if (txcfg->pstat->useCts2self) {
					*bCts2SelfEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
					priv->rts_enable_reason = (RTS_RC_HWRTSEN | RTS_RC_CTS2SELF | RTS_RC_HW_CTRL);
#endif
				}
			} else if (is_MCS_rate(txcfg->tx_rate) && ((txcfg->pstat->IOTPeer != HT_IOT_PEER_INTEL) || !txcfg->pstat->no_rts)) {
				...
				if (priv->pmib->miscEntry.auto_rts_mode == 1 && priv->cochannel_to) {
					...
					*bHwRts = FALSE;
#ifdef _DEBUG_RTL8192CD_
					priv->rts_enable_reason = (RTS_RC_RTSEN | RTS_RC_COCHANNEL);
#endif
				} else
#endif
				{
...
static void RtsCheckAC(struct rtl8192cd_priv *priv, struct tx_insn *txcfg, BOOLEAN *bRtsEnable, BOOLEAN *bCts2SelfEnable, BOOLEAN *bHwRts)
{
	if ((priv->pshare->rf_ft_var.cca_rts) && (txcfg->pstat->vht_cap_len > 0)) {
		...
		*bRtsEnable = TRUE;
#ifdef _DEBUG_RTL8192CD_
		priv->rts_enable_reason = (RTS_RC_RTSEN | RTS_RC_VHT_CCA);
#endif
	}
}
#endif
...
#if 1
	if (priv->pshare->rf_ft_var.rts_init_rate != 0xff) {
...
void set_ampdu_max_len_reg(struct rtl8192cd_priv *priv, unsigned char flag) {
	#define STATE_AMPDU_MAX_LENGTH_DEFAULT 1
	#define STATE_AMPDU_MAX_LENGTH_MULTISTA 2
	#define REG_AMPDU_MAX_LENGTH_DEFAULT_VALUE 0xffffffff
	#define AMPDU_MAX_LENGTH_5G 0x6400  // 1600*16  (1518+header size =1600)
	#define AMPDU_MAX_LENGTH_2G 0x3200  // 1600*8   (1518+header size =1600)
	#define REG_AMPDU_MAX_LENGTH 0x458

	switch(flag) {
		case STATE_AMPDU_MAX_LENGTH_MULTISTA:
			if (priv->pmib->dot11RFEntry.phyBandSelect & PHY_BAND_5G){
				RTL_W32(REG_AMPDU_MAX_LENGTH, AMPDU_MAX_LENGTH_5G);
				//	pdesc_data->maxAggNum = 8;			// max agg# 16	
			}else{
				RTL_W32(REG_AMPDU_MAX_LENGTH, AMPDU_MAX_LENGTH_2G);
				//	pdesc_data->maxAggNum = 4;			// max agg# 8
			}
			break;
		case STATE_AMPDU_MAX_LENGTH_DEFAULT:
		default:
			RTL_W32(REG_AMPDU_MAX_LENGTH, REG_AMPDU_MAX_LENGTH_DEFAULT_VALUE);				
			break;
	}
}


inline void set_ampdu_max_len(struct rtl8192cd_priv *priv) 
{
#define STATE_AMPDU_MAX_LENGTH_DEFAULT 1
#define STATE_AMPDU_MAX_LENGTH_MULTISTA 2
#define REG_AMPDU_MAX_LENGTH_DEFAULT_VALUE 0xffffffff
...
#define REG_AMPDU_MAX_LENGTH 0x458
			...
			if (priv->pshare->swq_numActiveSTA > LOWAGGRESTA 
#ifdef CONFIG_VERIWAVE_CHECK				
				&& !is_veriwave_testing(priv)
#endif					
			){				
				flag = STATE_AMPDU_MAX_LENGTH_MULTISTA;
			}
			else {
				flag = STATE_AMPDU_MAX_LENGTH_DEFAULT;
			}

			if(flag != priv->pshare->maxpktlen_flag) {
				set_ampdu_max_len_reg(priv, flag);
				priv->pshare->maxpktlen_flag = flag;
			}
#endif						

}

void set_legacy_sc(struct rtl8192cd_priv *priv, PTX_DESC_DATA_88XX pdesc_data, struct tx_insn *txcfg)
{
	char bg_txshort = 0;
	if (is_CCK_rate(txcfg->tx_rate) && (txcfg->tx_rate != 2)) {
		if ((priv->pmib->dot11BssType.net_work_type & WIRELESS_11G) &&
...
//	BOOLEAN			bCts2SelfEnable;
	...
	if (is_MCS_rate(txcfg->tx_rate)) {  // HT rates
		...
		if (
			...
			if (txcfg->pstat->ADDBA_ready[TID] && !txcfg->pstat->low_tp_disable_ampdu) {
				...
				set_ampdu_max_len(priv);
#endif	
#ifdef TC_CONCURRENT_FT
			if (GET_CHIP_VER(priv)==VERSION_8822B && priv->pshare->tc_msta_dual && (priv->pshare->th_mode==1))
		...
		txRate = get_rate_index_from_ieee_value((UINT8)txcfg->tx_rate);	
		set_legacy_sc(priv, pdesc_data, txcfg);
...
#ifdef _DEBUG_RTL8192CD_
		priv->rts_enable_reason &= ~RTS_RC_RTSEN;
		priv->rts_enable_reason |= RTS_RC_CTS2SELF;
#endif
	}

#ifdef TC_CONCURRENT_FT
	if (GET_CHIP_VER(priv)==VERSION_8197F && priv->pshare->tc_msta_mode) {
		pdesc_data->RTSEn = FALSE;
	...
	if (priv->pmib->miscEntry.auto_rts_mode == 2) {
		...
		pdesc_data->HWRTSEn = FALSE;
#ifdef _DEBUG_RTL8192CD_
		priv->rts_enable_reason = (RTS_RC_RTSEN | RTS_RC_AUTO_MODE);
#endif
	}
		...
		pdesc_data->HWRTSEn = FALSE;
#ifdef _DEBUG_RTL8192CD_
		priv->rts_enable_reason = (RTS_RC_CTS2SELF | RTS_RC_AUTO_MODE);
#endif
	}
#endif
...
//eric-7260
	...
	if (priv->pmib->dot11RFEntry.bcnagc != 0) {
		set_bcnagc(priv, txcfg, pdesc_data);
	}
...
#ifdef CONFIG_PCI_HCI
#ifdef CONFIG_WLAN_HAL
void rtl88XX_signin_txdesc(
			struct rtl8192cd_priv *priv, 
			struct tx_insn *txcfg, 
			unsigned char convHdr)
{
...
int get_swq_turbo_thd(struct rtl8192cd_priv *priv)
{
	int result;
...
#endif
	UINT32 tri_time;
	int assoc_num = priv->pshare->total_assoc_num;
	int i;
    int swq_turbo_thd;
	...
	if (priv->pshare->swq_use_hw_timer) {
...
#endif
		{
			...
			if ((MBPS_TO_BIT(10) < pstat->rx_avarage) && ((pstat->tx_avarage * 5) <  pstat->rx_avarage)) {
				...
				if (pstat->swq.swq_en[q_num] && assoc_num <= swq_turbo_thd) {
					if (pstat->tx_avarage < MBPS_TO_BIT(5) || MBPS_TO_BIT(160) < pstat->tx_avarage) { /* TP < 5M or TP > 160M */
						pstat->swq.swq_en[q_num] = 0; /*disable swq*/
						if (pstat->swq.swq_timer_id[q_num])
							rtl8192cd_swq_deltimer(priv, pstat, q_num);
						if (pstat->swq.swq_empty[q_num]) {
				...
				} else if (assoc_num > swq_turbo_thd || (KBPS_TO_BIT(5500) < pstat->tx_avarage && pstat->tx_avarage < MBPS_TO_BIT(155))) { /* 5.5M < TP < 155M*/
						pstat->swq.swq_en[q_num] = 1; /*enable swq*/
						pstat->swq.swq_prev_timeout[q_num] = 1;
						pstat->swq.swq_timeout_change[q_num] = 0;
						pstat->swq.swq_keeptime[q_num] = 0;
    ...
    if(need_deque) {
        ...
        if(rtl8192cd_swq_bdfull(priv, pstat->swq.q_aggnum[q_num], q_num)) {                   
            ...
            if(priv->pshare->swq_use_hw_timer && pstat->swq.swq_empty[q_num] == 0) {
                priv->pshare->swq_numActiveSTA++;
                if(priv->pshare->swq_numActiveSTA == (swq_turbo_thd + 1)) {
...
//					*((unsigned short *)(skb->data+MACADDRLEN*2)) != __constant_htons(ETH_P_8021Q) &&
	...
	if(pstat && IPV4_TCP_ACK_PACKET(skb->data) && (tx_flag != TX_NO_TCPACK_ACC))
		pstat->tcpack_pkts++;

#ifdef TCP_ACK_ACC
	if (pstat && priv->pmib->miscEntry.tcpack_acc && pstat->tcpack_acc_en && (tx_flag != TX_NO_TCPACK_ACC)) {
		if (tx_tcp_ack_accelerate(priv, pstat, skb)<0)
			goto stop_proc;
	}
#endif
...
xmit_out:
	...
	if(pstat && IPV4_TCP_ACK_PACKET(skb->data))
		pstat->tcpack_pkts_sent++;
...
#ifdef TC_CONCURRENT_FT				
				|| priv->pshare->tc_msta_dual
#endif				
				) && IS_UDP_PROTO(skb->data)) {
				if (GET_CHIP_VER(priv) == VERSION_8814A) {
...
//	PHCI_TX_DMA_MANAGER_88XX	ptx_dma;
	...
	if (GetFrameType(txcfg->phdr) == WIFI_DATA_TYPE)
		priv->ext_stats.tx_data_pkts++;
#endif
...
void send_tcp_ack(struct rtl8192cd_priv *priv, struct tcp_session *tcp_ses, struct stat_info *pstat, unsigned long current_time)
{
	struct sk_buff *pskb;
	pskb = tcp_ses->buf_ack;
	if (pskb) {
		tcp_ses->buf_ack = NULL;
		tcp_ses->timestamp = current_time;

		__rtl8192cd_start_xmit(pskb, priv->dev, TX_NO_TCPACK_ACC);
		pstat->tcpack_to_cnt1++;
	}
}

void check_tcp_ack_timeout(struct rtl8192cd_priv *priv, unsigned int ack_anyway)
{
	struct list_head *phead, *plist;
	struct stat_info *pstat;
	struct tcp_session *tcp_ses;
	...
	if(priv->pmib->dot11RFEntry.phyBandSelect == PHY_BAND_2G)
		timeout = priv->pmib->miscEntry.tcpack_to_2g;
	else
		timeout = priv->pmib->miscEntry.tcpack_to;
	
	unsigned long current_time = 0;
	unsigned int delay_start_time = 0;
	unsigned int del_section_time = 0;
	unsigned int tcpack_to = 0;
	if(priv->pmib->miscEntry.tcpack_use_hw_timer){
		// in us
		current_time = RTL_R32(TSFTR1);
		delay_start_time = priv->pmib->miscEntry.tcpack_delay_start*1000;
		del_section_time = 2000*1000;// 2sec
		tcpack_to = timeout*1000;
	} else {
		current_time = jiffies;
		delay_start_time = RTL_MILISECONDS_TO_JIFFIES(priv->pmib->miscEntry.tcpack_delay_start);
		del_section_time = RTL_SECONDS_TO_JIFFIES(2);
		tcpack_to = RTL_MILISECONDS_TO_JIFFIES(timeout);
	}
		...
		for (i = 0; i < TCP_SESSION_MAX_ENTRY; i++) {
			...
			if (tcp_ses->used) {
				if (!priv->pmib->miscEntry.tcpack_acc) {
					send_tcp_ack(priv, tcp_ses, pstat, current_time);
					del_tcp_session(pstat, tcp_ses);
				}
				else if (priv->pmib->miscEntry.tcpack_delay_start != 0 &&
							current_time - tcp_ses->boot_time < delay_start_time) {
					continue;
				}
				else if (current_time - tcp_ses->timestamp > del_section_time) {
					send_tcp_ack(priv, tcp_ses, pstat, current_time);
					del_tcp_session(pstat, tcp_ses);
				}
				else if (current_time - tcp_ses->timestamp > tcpack_to) {
					send_tcp_ack(priv, tcp_ses, pstat, current_time);
				}
...
int tx_tcp_ack_accelerate(struct rtl8192cd_priv *priv, struct stat_info *pstat, struct sk_buff *skb)
{
	...
	pstat->tcpack_buf_cnt = ack_th;

	unsigned long current_time = 0;
	unsigned int delay_start_time = 0;
	unsigned int tcpack_to = 0;
	if(priv->pmib->miscEntry.tcpack_use_hw_timer == 1){
		// in us
		current_time = RTL_R32(TSFTR1);
		delay_start_time = priv->pmib->miscEntry.tcpack_delay_start*1000;
		tcpack_to = timeout*1000;
	} else if(priv->pmib->miscEntry.tcpack_use_hw_timer == 0){
		current_time = jiffies;
		delay_start_time = RTL_MILISECONDS_TO_JIFFIES(priv->pmib->miscEntry.tcpack_delay_start);
		tcpack_to = RTL_MILISECONDS_TO_JIFFIES(timeout);
	}
	...
	do {
		...
		if (tcp_ses != NULL) {
			if(priv->pmib->miscEntry.tcpack_delay_start != 0 && !tcp_ses->is_delay_start_ok) {
				if(priv->pmib->miscEntry.tcpack_use_hw_timer == 2){
					current_time = RTL_R32(TSFTR1);
					delay_start_time = priv->pmib->miscEntry.tcpack_delay_start*1000;
				}
				
				if (current_time - tcp_ses->boot_time < delay_start_time) {				
					tcp_ses->timestamp = current_time;
					break;
				} else {
					tcp_ses->is_delay_start_ok = 1;
				}
			}
			...
			if(priv->pmib->miscEntry.tcpack_use_hw_timer == 2){
				...
				//check if timeout
				if (current_time - tcp_ses->timestamp < tcpack_to) {
					tcp_ses->ack_cnt++;
					tcp_ses->hit++;
					if(ack_th && tcp_ses->ack_cnt >= ack_th) {
						tcp_ses->ack_cnt = 0;
			...
			if (tcp_ses) {
				...
				tcp_ses->ack_seq = seq;
				tcp_ses->timestamp = current_time;
				tcp_ses->boot_time = current_time;
				tcp_ses->is_delay_start_ok = 0;
			}
		}
	} while (0);
