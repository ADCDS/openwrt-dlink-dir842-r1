...
int __DOT11_Process_Disconnect_Req(struct net_device *dev, struct iw_point *data)
{
...
#endif
	{
		DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "A STA is rejected - %02X:%02X:%02X:%02X:%02X:%02X, Reason=%d\n",
								pstat->hwaddr[0],pstat->hwaddr[1],pstat->hwaddr[2],pstat->hwaddr[3],pstat->hwaddr[4],pstat->hwaddr[5], Disconnect_Req->Reason);
...
#endif
					// data pkt MIC fail
					priv->del_sta_info.branch_code = 11;
					set_del_sta_info(priv, pstat_del->hwaddr, _RSON_MIC_FAILURE_);
		...
		case DOT11_EVENT_FT_KEY_EXPIRE_IND:
			{
				...
				// del_sta branch 12, 802.11r del sta when key expire
				priv->del_sta_info.branch_code = 12;
