...
#include <asm/mipsregs.h>	/* read_c0_count() — IRQ-off-safe wall-clock bound */

/* SPIC FLASH Headder */
#include "spi-sheipa.h"

/* --- flash-boot hang instrumentation (TEMPORARY, remove once diagnosed) ---
 * The intermittent flash-boot hang is a TOTAL freeze (no ping/serial/printk),
 * consistent with the CPU stalling on a memory-mapped flash access when the
 * controller is wedged. printk/softlockup are useless there. Instead poll-write
 * one raw char directly to UART0 (8197F dw-apb: base 0xB8147000, TX @ +0x024,
 * LSR @ +0x014, THRE=0x20 — see arch/mips/realtek/prom.c) at each flash-op
 * class, but ONLY when the class changes, so volume/timing-disturbance stay low.
 * On a freeze, the last unmatched capital names the stuck path:
 *   R auto_read   C cmd-read   W auto_write   D cmd-write   e erase-cmd
 */
/* SPI-freeze UART breadcrumb tracer — set to 1 to diagnose a command-mode wedge
 * (writes the stuck-path marker char direct to UART0 on each SPI op). Ships DISABLED. */
#define SPI_HANG_TRACE 0
#if SPI_HANG_TRACE
static inline void spi_bc_putc(char c)
{
	volatile unsigned char *lsr = (volatile unsigned char *)0xB8147014;
	volatile unsigned char *tx  = (volatile unsigned char *)0xB8147024;
...
static int spi_bc_last = -1;
static inline void spi_bc_op(int op, char c)
{
	if (op != spi_bc_last) {
		spi_bc_last = op;
		spi_bc_putc(c);
				...
				enum spi_flash_byte_num byte_num)
{
	...
	/* Wait for RX FIFO non-empty (RFNE) before popping the data register.
	 * The hardware dr[] read BLOCKS the CPU until a word is present. Under
	 * sustained command read/write switching — jffs2 overlay writes
	 * interleaved with reads on a flash boot — the controller intermittently
	 * wedges and never fills the FIFO, so the load stalls forever with no IRQ
	 * or printk possible: the silent total freeze that UART breadcrumbs caught
	 * as "...DCDCDC" then dead on a 'C' (command read). enable_cs_read only
	 * waits on the BUSY bit (bounded, so no message), which clears without the
	 * RX data being ready. Bound the wait on RFNE and bail with 0 + a FIFO
	 * reset so the caller/mtd layer retries from a clean state instead of
	 * hanging the box. */
	{
		unsigned int t0 = read_c0_count();
		while ((spi_flash_map->sr & (1u << bfoSPI_FLASH_SR_RFNE)) == 0) {
			if ((read_c0_count() - t0) > 10000000u) {
				pr_err_ratelimited("spi-sheipa: getdr RFNE timeout — controller wedged, aborting read\n");
				spi_flash_map->ssienr = 0;
				return 0;
			}
			cpu_relax();
...
void spi_flash_wait_busy(struct sheipa_spi *dev)
{
	...
	unsigned int timeout = 50000000;	/* runs under spin_lock_irqsave (IRQs off)
						 * so jiffies can't advance — bound with a loop
						 * counter (~100ms of tight polling) instead of
						 * spinning forever when the controller wedges. */
	...
	pr_err("spi-sheipa: spi_flash_wait_busy TIMEOUT (controller wedged)\n");
}
...
{
	/* WIP poll. Each iteration is a full status-read command whose own transfer
	 * wait is bounded, but that inner bound can be ~0.25s when the controller is
	 * wedged, so a plain iteration counter would still spin for minutes. Bound on
	 * the free-running CP0 Count (CPU_clk/2, safe to read with IRQs off) instead:
	 * ~2-3s wall clock, then bail rather than hang the CPU forever. */
	unsigned int start = read_c0_count();

	/* Check flash is in write progress or not */
	while ((flash_get_status(dev) & 0x1)) {
		if ((read_c0_count() - start) > 1000000000u) {
			pr_err("spi-sheipa: flash_wait_busy TIMEOUT (WIP stuck)\n");
			break;
		}
		cpu_relax();
	}
		       ...
		       uint32_t offset)
{
	...
	spi_bc_op(3, 'W');	/* about to do memory-mapped auto writes */
	for (i = 0; i < cnt; i++) {
		memcpy(&data, buf, 4);
		buf += 4;
		*addr = data;
		      ...
		      uint32_t offset)
{
	...
	spi_bc_op(1, 'R');	/* about to do memory-mapped auto reads */
	cnt = len / 4;
	for (i = 0; i < cnt; i++) {
		data = *addr;
		memcpy(buf, &data, 4);
				    ...
				    uint32_t type, uint8_t cmd)
{
	...
	case M25P80_AUTO_WRITE:
		/* Commands with no extended (dual/quad) handling land here, e.g.
		 * WRDI (0x04) which the m25p80 layer issues after every write/erase.
		 * It is a harmless no-op (SPI-NOR auto-clears WEL after program/erase),
		 * so demote to pr_debug — at pr_info it floods the console during
		 * jffs2 overlay writes when booted from flash. */
		pr_debug("flash: no extended op for cmd 0x%02X, flash_mode %d\n",
		       cmd, flash_mode);
		break;
	case M25P80_QUAD:
			       ...
			       uint32_t len)
{
	...
	if (!mode) {
		spi_bc_op(4, 'D');	/* command-mode FIFO write */
...
static inline void do_spi_recv(struct sheipa_spi *dws, u8 *buf, uint32_t len)
{
	...
	if (!mode) {
		spi_bc_op(2, 'C');	/* command-mode FIFO read */
		if (len <= FIFO_SIZE) {
		...
		/* Re-establish auto (memory-mapped) read mode before the read.
		 * A preceding command-mode operation (e.g. a jffs2 erase/program on
		 * the overlay partition, interleaved with squashfs reads during a
		 * flash boot) can leave the SPI controller in a state where the
		 * memory-mapped '*addr' read stalls the CPU forever — there is no
		 * software wait loop there to time out, so the box hangs silently.
		 * Re-initing auto mode here forces a known-good state first. */
		auto_init(dws);
...
/*
 * The controller is a flash controller, not a general-purpose SPI master: a
 * command is issued as an opcode plus address/dummy descriptor, and the data
 * phase is driven separately. The spi-mem interface hands us exactly that
 * shape, whereas the generic spi_message fallback would split the opcode and
 * the address into separate transfers and the command decode would see
 * garbage. So implement mem_ops and do not offer transfer_one_message at all.
 */
static bool sheipa_spi_supports_op(struct spi_mem *mem,
				   const struct spi_mem_op *op)
{
	if (!spi_mem_default_supports_op(mem, op))
		return false;

	/* single-lane only; the x2/x4 paths of this controller are unused */
	...
	/*
	 * Reject SFDP discovery: the command decode below has no descriptor for
	 * it, and every flash this controller is paired with is covered by the
	 * spi-nor ID table.
	 */
	if (op->cmd.opcode == SPINOR_OP_RDSFDP)
		return false;

	return true;
}

static void sheipa_spi_set_speed(struct sheipa_spi *dws, struct spi_device *spi)
{
	...
	clk_div = (dws->max_freq % spi->max_speed_hz) ?
		  dws->max_freq / spi->max_speed_hz + 1 :
		  dws->max_freq / spi->max_speed_hz;
	/*
	 * Floor the divider (=> cap SCK) for timing margin against the
	 * command-mode controller wedge. ~28 puts SCK near 8 MHz.
	 */
	if (clk_div < 28)
		clk_div = 28;
...
static int sheipa_spi_exec_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	struct sheipa_spi *dws = spi_controller_get_devdata(mem->spi->controller);
	unsigned long flags;
	u8 cmdbuf[8] = { };
	unsigned int i;

	if (op->data.nbytes && !op->data.buf.in && !op->data.buf.out)
		return -EINVAL;

	if (mem->spi->max_speed_hz > dws->max_freq) {
		dev_err(&mem->spi->dev, "unsupported freq %dHz\n",
			mem->spi->max_speed_hz);
		return -EINVAL;
	}

	/*
	 * Command descriptor consumed by do_spi_cmd(): opcode, big-endian
	 * address, then flash mode / dummy cycles / access type.
	 */
	cmdbuf[0] = op->cmd.opcode;
	for (i = 0; i < op->addr.nbytes; i++)
		cmdbuf[1 + i] = op->addr.val >> (8 * (op->addr.nbytes - i - 1));
	cmdbuf[5] = 0;
	cmdbuf[6] = op->dummy.nbytes;
	cmdbuf[7] = 0;

	spin_lock_irqsave(&dws->ctlr->bus_lock_spinlock, flags);

	sheipa_spi_set_speed(dws, mem->spi);

	do_spi_cmd(dws, cmdbuf, sizeof(cmdbuf));

	if (op->data.nbytes) {
		if (op->data.dir == SPI_MEM_DATA_IN)
			do_spi_recv(dws, op->data.buf.in, op->data.nbytes);
		else
			do_spi_send(dws, op->data.buf.out, op->data.nbytes);
	}

	/*
	 * Fully disable the controller before releasing the bus. Leaving
	 * ssienr = 1 lets the next command's setup race a not-yet-settled state
	 * machine; under the sustained back-to-back writes and reads of a jffs2
	 * overlay copy that intermittently wedges it so hard that even MMIO
	 * status reads block.
	 */
	((struct spi_flash_portmap *)dws->regs)->ssienr = 0;
	udelay(2);

	spin_unlock_irqrestore(&dws->ctlr->bus_lock_spinlock, flags);

	return 0;
}

static const struct spi_controller_mem_ops sheipa_spi_mem_ops = {
	.supports_op = sheipa_spi_supports_op,
	.exec_op = sheipa_spi_exec_op,
};

static int dw_spi_setup(struct spi_device *spi)
{
	struct sheipa_spi *dws = spi_controller_get_devdata(spi->controller);
...
static int sheipa_spi_probe(struct platform_device *pdev)
{
	...
	ctlr = spi_alloc_host(&pdev->dev, sizeof(*spi));
	if (ctlr == NULL) {
	...
	ctlr->mem_ops = &sheipa_spi_mem_ops;
	...
	ctlr->min_speed_hz = spi->max_freq/8;
	ctlr->max_speed_hz = spi->max_freq;
...
static void sheipa_spi_remove(struct platform_device *pdev)
{
	/* everything the driver holds is devm-managed */
}
