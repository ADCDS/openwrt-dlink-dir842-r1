...
static int dw_spi_transfer(struct spi_master *master, struct spi_message *msg)
{
		...
		if (spi->max_speed_hz != spi_flash_para->spi_flash_sclk) {
			if (spi->max_speed_hz > dws->max_freq) {
				dev_err(&spi->dev, "unsupported freq %dHz\n",
					spi->max_speed_hz);
				/* must not return with bus_lock_spinlock held +
				 * IRQs off, or every later flash access deadlocks */
			...
			/* Floor the divider (=> cap the SCK) for timing margin
			 * against the command-mode R/W controller wedge. A larger
			 * divider means a slower clock; ~28 puts SCK near 8MHz. */
			if (clk_div < 28)
				clk_div = 28;
	...
	/* Fully deassert/disable the controller before releasing the bus. Without
	 * this, ssienr is left =1 after a read/write and the NEXT command
	 * transaction's setup races against a not-fully-settled controller; under
	 * the sustained back-to-back command write+read of a jffs2 overlay copy
	 * this intermittently wedges the state machine so hard that even MMIO
	 * status reads block (the silent DCDCDC freeze). A clean ssienr=0 + short
	 * settle between messages gives it a defined idle boundary. */
