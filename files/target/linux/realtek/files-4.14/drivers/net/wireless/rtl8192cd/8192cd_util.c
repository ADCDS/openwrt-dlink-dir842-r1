...
/* OpenWrt port: kernel 4.10 split the signal API out of <linux/sched.h>.  This
 * file calls kill_pid(_wlanapp_pid, SIGIO, 1) etc. (5297, 5457, 5470, 12437);
 * without this header it is implicitly declared, so gcc assumes int(...) instead
 * of the real int(struct pid *, int, int).  The globals really are `struct pid *`
 * (8192cd_util.c:81, 8192cd.h:4185), so this just restores the true prototype. */
	...
	if (pstat) {
		if ((mode == 1) && (isBasicRate == 0) && pstat->ht_cap_len && (!should_restrict_Nrate(priv, pstat))) {
			...
			if(pstat->vht_cap_len) {
				int ss = ((le32_to_cpu(pstat->vht_cap_buf.vht_support_mcs[0])>>4)&3);
				if(ss != 3)	{	
					switch(ss) {
						case 1:
							hirate = VHT_RATE_ID + 38;
							break;
						case 2:
							hirate = VHT_RATE_ID + 39;
							break;
						default:
							hirate = VHT_RATE_ID + 37;
					};
				} else	{
					ss = ((le32_to_cpu(pstat->vht_cap_buf.vht_support_mcs[0])>>2)&3) ;
					if(ss != 3) {
						switch(ss) {
							case 1:
								hirate = VHT_RATE_ID + 28;
								break;
							case 2:
								hirate = VHT_RATE_ID + 29;
								break;
							default:
								hirate = VHT_RATE_ID + 27;
						};					
					} else {
						ss = ((le32_to_cpu(pstat->vht_cap_buf.vht_support_mcs[0]))&3) ;
						switch(ss) {
							case 1:
								hirate = VHT_RATE_ID + 8;
								break;
							case 2:
								hirate = VHT_RATE_ID + 9;
								break;
							case 0:
								hirate = VHT_RATE_ID + 7;
								break;
							default:
								;
						};						
...
static struct channel_list reg_channel_5g_full_band[] = {
	...
	/* ETSI */		{{36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116, 120, 124, 128, 132, 136, 140, 144}, 20},
	...
	/* Global */		{{36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116, 136, 140, 144, 149, 153, 157, 161, 165}, 21},
	/* World_wide */	{{36, 40, 44, 48, 52, 56, 60, 64, 100, 104, 108, 112, 116, 136, 140, 144, 149, 153, 157, 161, 165}, 21},
...
void cnt_assoc_num(struct rtl8192cd_priv *priv, struct stat_info *pstat, int act, char *func)
{
		...
		if (priv->assoc_num <= NUM_STAT) {
            ...
            if (pstat->IOTPeer == HT_IOT_PEER_VERIWAVE)
            {
            	if(priv->pshare->veriwave_sta_num == 0) {
					...
					ODM_CmnInfoUpdate(ODMPTR, ODM_CMNINFO_ABILITY, ODMPTR->SupportAbility & ~ODM_COMP_ADAPTIVE_SOML);
	...
	if (GET_CHIP_VER(priv) == VERSION_8192D) {
		...
		if (((struct rtl8192cd_priv *)if_priv[0])->pshare->tc_msta_mode && ((struct rtl8192cd_priv *)if_priv[1])->pshare->tc_msta_mode) {
...
void init_priv_skb_buf(struct rtl8192cd_priv *priv)
{
	...
	if(idx == (0^WLANIDX)){ 
			...
			panic_printk("NUM_RX_DESC[%d] NUM_TX_DESC[%d]\n",NUM_RX_DESC, NUM_TX_DESC);
			panic_printk("MAX_RX_BUF_LEN[%d]\n",MAX_RX_BUF_LEN);
			panic_printk("############################\n");
	}else{
			panic_printk("##########(wlan1)##########\n");
			panic_printk("SKB_BUF_SIZE[%d] MAX_SKB_NUM[%d]\n",skb_size, skb_num);
			panic_printk("NUM_RX_DESC[%d] NUM_TX_DESC[%d]\n",NUM_RX_DESC_2G, NUM_TX_DESC);
			panic_printk("MAX_RX_BUF_LEN[%d]\n",MAX_RX_BUF_LEN_1);
			panic_printk("###########################\n");
	}
...
extern int get_swq_turbo_thd(struct rtl8192cd_priv *priv);
	...
	while (plist != phead) {
		...
		if (r0kh->key_expire_to > 0 && --r0kh->key_expire_to == 0) {
			...
			if (pstat) {
				...
				// del_sta branch 23, 802.11r del sta when key expire
				priv->del_sta_info.branch_code = 23;
...
struct r0_key_holder *search_r0kh(struct rtl8192cd_priv *priv, unsigned char *sta_addr)
{
	...
	struct r0_key_holder *ret_r0kh = NULL;
	struct list_head    *phead, *plist;
	unsigned long flags;
	if (!netif_running(priv->dev)){
		return NULL;		
...
struct r1_key_holder *search_r1kh_by_pmkid(struct rtl8192cd_priv *priv, unsigned char *pmkid, unsigned int id)
{
	...
	struct r1_key_holder *ret_r1kh=NULL;
	struct list_head    *phead, *plist;
	unsigned long flags;
...
void set_del_sta_info(struct rtl8192cd_priv *priv, unsigned char *hwaddr, unsigned char reason_code)
{
	struct stat_info *pstat;

	memcpy(priv->del_sta_info.addr, hwaddr, MACADDRLEN);
	priv->del_sta_info.reason_code = reason_code;

	pstat = get_stainfo(priv, hwaddr);
	if (pstat == NULL)
		goto print_log;

	priv->del_sta_info.tx_rate = pstat->current_tx_rate;
	priv->del_sta_info.rx_rate = pstat->rx_rate;
	priv->del_sta_info.tx_tp = pstat->current_tx_bytes;
	priv->del_sta_info.rx_tp = pstat->current_rx_bytes;
	priv->del_sta_info.rssi = pstat->rssi;

print_log:

	LOG_MSG("Last Deleted Station Info...\n");
	LOG_MSG("  addr: %02x%02x%02x%02x%02x%02x\n", 
		priv->del_sta_info.addr[0], priv->del_sta_info.addr[1], 
		priv->del_sta_info.addr[2], priv->del_sta_info.addr[3], 
		priv->del_sta_info.addr[4], priv->del_sta_info.addr[5]);
#ifdef RTK_AC_SUPPORT  //vht rate
	if (priv->del_sta_info.tx_rate >= VHT_RATE_ID) {
		LOG_MSG("  tx_rate: VHT NSS%u-MCS%u\n", 
			((priv->del_sta_info.tx_rate - VHT_RATE_ID) / 10) + 1, 
			(priv->del_sta_info.tx_rate - VHT_RATE_ID) % 10);
	} else
#endif
	if (is_MCS_rate(priv->del_sta_info.tx_rate)) {
		LOG_MSG("  tx_rate: MCS%u\n", (priv->del_sta_info.tx_rate - HT_RATE_ID));
	} else {
		LOG_MSG("  tx_rate: %d\n", priv->del_sta_info.tx_rate / 2);
	}

#ifdef RTK_AC_SUPPORT
	if (priv->del_sta_info.rx_rate >= VHT_RATE_ID) {
		LOG_MSG("  rx_rate: VHT NSS%u-MCS%u\n", 
			((priv->del_sta_info.rx_rate - VHT_RATE_ID) / 10) + 1, 
			(priv->del_sta_info.rx_rate - VHT_RATE_ID) % 10);
	} else
#endif
	if (is_MCS_rate(priv->del_sta_info.rx_rate)) {
		LOG_MSG("  rx_rate: MCS%u\n", (priv->del_sta_info.rx_rate - HT_RATE_ID));
	} else {
		LOG_MSG("  rx_rate: %d\n", priv->del_sta_info.rx_rate / 2);
	}

	LOG_MSG("  tx_tp: %u bytes\n", priv->del_sta_info.tx_tp);
	LOG_MSG("  rx_tp: %u bytes\n", priv->del_sta_info.rx_tp);
	LOG_MSG("  rssi: %u\n", priv->del_sta_info.rssi);
	LOG_MSG("  reason: %u\n", priv->del_sta_info.reason_code);
	LOG_MSG("  branch: %u\n", priv->del_sta_info.branch_code);

#if 0 // debug info
#ifdef RTK_AC_SUPPORT  //vht rate
	if (priv->del_sta_info.tx_rate >= VHT_RATE_ID) {
		panic_printk("tx_rate: VHT NSS%u-MCS%u\n", 
			((priv->del_sta_info.tx_rate - VHT_RATE_ID) / 10) + 1, 
			(priv->del_sta_info.tx_rate - VHT_RATE_ID) % 10);
	}
	else
#endif
	if (is_MCS_rate(priv->del_sta_info.tx_rate)) {
		panic_printk("tx_rate: MCS%u\n", (priv->del_sta_info.tx_rate - HT_RATE_ID));
	}
	else
	{
		panic_printk("tx_rate: %d\n", priv->del_sta_info.tx_rate / 2);
	}

#ifdef RTK_AC_SUPPORT
	if (priv->del_sta_info.rx_rate >= VHT_RATE_ID) {
		panic_printk("rx_rate: VHT NSS%u-MCS%u\n", 
			((priv->del_sta_info.rx_rate - VHT_RATE_ID) / 10) + 1, 
			(priv->del_sta_info.rx_rate - VHT_RATE_ID) % 10);
	}
	else
#endif
	if (is_MCS_rate(priv->del_sta_info.rx_rate)) {
		panic_printk("rx_rate: MCS%u\n", (priv->del_sta_info.rx_rate - HT_RATE_ID));
	}
	else
	{
		panic_printk("rx_rate: %d\n", priv->del_sta_info.rx_rate / 2);
	}

	panic_printk("tx_tp: %u bytes\n", priv->del_sta_info.tx_tp);
	panic_printk("rx_tp: %u bytes\n", priv->del_sta_info.rx_tp);
	panic_printk("rssi: %u\n", priv->del_sta_info.rssi);
	panic_printk("reason: %u\n", priv->del_sta_info.reason_code);
	panic_printk("branch: %u\n", priv->del_sta_info.branch_code);
#endif
}

void set_leave_sta_info(struct rtl8192cd_priv *priv, unsigned char *hwaddr, unsigned char reason_code)
{
	struct stat_info *pstat;

	memcpy(priv->leave_sta_info.addr, hwaddr, MACADDRLEN);
	priv->leave_sta_info.reason_code = reason_code;
	
	pstat = get_stainfo(priv, hwaddr);
	if (pstat == NULL)
		goto print_log;

	priv->leave_sta_info.tx_rate = pstat->current_tx_rate;
	priv->leave_sta_info.rx_rate = pstat->rx_rate;
	priv->leave_sta_info.tx_tp = pstat->current_tx_bytes;
	priv->leave_sta_info.rx_tp = pstat->current_rx_bytes;
	priv->leave_sta_info.rssi = pstat->rssi;

print_log:

	LOG_MSG("Last Leave Station Info...\n");
	LOG_MSG("  addr: %02x%02x%02x%02x%02x%02x\n", 
		priv->leave_sta_info.addr[0], priv->leave_sta_info.addr[1], 
		priv->leave_sta_info.addr[2], priv->leave_sta_info.addr[3], 
		priv->leave_sta_info.addr[4], priv->leave_sta_info.addr[5]);
#ifdef RTK_AC_SUPPORT  //vht rate
	if (priv->leave_sta_info.tx_rate >= VHT_RATE_ID) {
		LOG_MSG("  tx_rate: VHT NSS%u-MCS%u\n", 
			((priv->leave_sta_info.tx_rate - VHT_RATE_ID) / 10) + 1, 
			(priv->leave_sta_info.tx_rate - VHT_RATE_ID) % 10);
	} else
#endif
	if (is_MCS_rate(priv->leave_sta_info.tx_rate)) {
		LOG_MSG("  tx_rate: MCS%u\n", (priv->leave_sta_info.tx_rate - HT_RATE_ID));
	} else {
		LOG_MSG("  tx_rate: %d\n", priv->leave_sta_info.tx_rate / 2);
	}

#ifdef RTK_AC_SUPPORT
	if (priv->leave_sta_info.rx_rate >= VHT_RATE_ID) {
		LOG_MSG("  rx_rate: VHT NSS%u-MCS%u\n", 
			((priv->leave_sta_info.rx_rate - VHT_RATE_ID) / 10) + 1, 
			(priv->leave_sta_info.rx_rate - VHT_RATE_ID) % 10);
	} else
#endif
	if (is_MCS_rate(priv->leave_sta_info.rx_rate)) {
		LOG_MSG("  rx_rate: MCS%u\n", (priv->leave_sta_info.rx_rate - HT_RATE_ID));
	} else {
		LOG_MSG("  rx_rate: %d\n", priv->leave_sta_info.rx_rate / 2);
	}

	LOG_MSG("  tx_tp: %u bytes\n", priv->leave_sta_info.tx_tp);
	LOG_MSG("  rx_tp: %u bytes\n", priv->leave_sta_info.rx_tp);
	LOG_MSG("  rssi: %u\n", priv->leave_sta_info.rssi);
	LOG_MSG("  reason: %u\n", priv->leave_sta_info.reason_code);
	LOG_MSG("  branch: %u\n", priv->leave_sta_info.branch_code);
