...
//-------------------------------------------------------------
// Support BT Coexist architecture
// current support: 8197F (software mail box)
//                  8822B (software mail box)    
//-------------------------------------------------------------
#ifdef CONFIG_BT_COEXIST
	#define ENABLE_BT_COEXIST
...
#else
#define SS_AUTO_CHNL_NHM_TO		RTL_MILISECONDS_TO_JIFFIES(200)
#endif
...
#define CONNECT_STA_INFO_TO		RTL_SECONDS_TO_JIFFIES(120)
...
/*adjusted for support AMSDU*/
...
#else
	#define MAX_RX_BUF_LEN_1 4200  //wlan1 97F
	#define MIN_RX_BUF_LEN_1 4200  //wlan1 97F
#endif
    #undef MAX_RX_BUF_LEN
    #undef MIN_RX_BUF_LEN
	#ifdef RX_BUFFER_GATHER
    ...
    #endif
	#else
	#define MAX_RX_BUF_LEN	12000   // wlan0 8822
	#define MIN_RX_BUF_LEN	4200   // wlan0 8822
    #endif
...
//-------------------------------------------------------------
#if defined(CONFIG_RTK_SMART_ROAMING) || defined(CONFIG_RTL_11V_SUPPORT)
...
#define SSID_PRIORITY_SUPPORT
	...
	#endif
#endif
#if (RTL8192CD_NUM_VWLAN>7)
/*if need extra interface to else purpose(non-VAP)
       keep priv->vap_id < 0  ; then this interafce will not affect MBSSID CAM*/
#error  RTL8192CD_NUM_VWLAN_DONT_MORE_THAN_7
#endif
...
#define WLAN_VDEV_FOR_V4		(1<<0)
#define WLAN_VDEV_FOR_V6		(1<<1)
#define WLAN_VDEV_FOR_V4_V6		(WLAN_VDEV_FOR_V4|WLAN_VDEV_FOR_V6)
#endif
...
/* RTS_RC_HWRTSEN / RTS_RC_RTSEN / RTS_RC_CTS2SELF:
 * 000: Do nothing
 * 001: always CTS2SELF
 * 010: always RTS
 * 011: RTS higher priority, always RTS
 * 100: Do nothing
 * 101: CTS2SELF if criteria/threshold is matched
 * 110: RTS if criteria/threshold is matched
 * 111: RTS if criteria/threshold is matched
 */
#define RTS_RC_HWRTSEN					BIT(31)
#define RTS_RC_RTSEN					BIT(30)
#define RTS_RC_CTS2SELF					BIT(29)

#define RTS_RC_AUTO_MODE				BIT(28)	// mode 2 = no RTS, mode 3 = always RTS, mode 4 = always CTS2SELF. (mode 1 equals to cochannel)
#define RTS_RC_VHT_CCA					BIT(27)	// when 802.11ac secondary channel CCA enable
#define RTS_RC_OUT_OF_RTS_THRESHOLD		BIT(26)	// mpdu_len > rts_threshold
#define RTS_RC_HT_PROTECTION			BIT(25)	// when 802.11n use MCS rate and there are legacy(a/b/g) OBSS STAs
#define RTS_RC_ERP_PROTECTION			BIT(24)	// when 802.11g use OFDM rate
#define RTS_RC_HT_MIMO_PS_DYNAMIC		BIT(23)	// when HT MIMO Dynamic power save is set
#define RTS_RC_HW_CTRL					BIT(22)	// hardware control
#define RTS_RC_COCHANNEL				BIT(21)	// when there are other APs in primary channel. (auto rts mode 1)
#endif

#define STA_CONNECT_SUCCESS				0
#define STA_CONNECT_CHALLENGE_FAIL		1	// Response message fail.
#define STA_CONNECT_NO_SUPP_ALG			2	// Authenticate algorithm not support.
#define STA_CONNECT_UNABLE_HANDLE		3	// BS resoruce insufficient.
#define STA_CONNECT_OTHER				4	// Other reason. (ex: ACL deny)
#define STA_CONNECT_FT_CHECK_FAIL		5	// 802.11r Fast BSS Transition Authentication IE check fail
#define STA_CONNECT_FT_CHECK_RRQ_FAIL	6	// 802.11r receives FT authentication sequence not supported by protocol
#define STA_CONNECT_OUT_OF_AUTH_SEQ		7	// Out of authenticate sequence number.
#define STA_CONNECT_INVALID_STA_FRAME	8	// The station is not recorded in the list
#define STA_CONNECT_AUTH_NOT_YET		9	// Authentication no yet before Association
