...
struct UINT8 * rtl_get_skb_ip_header(struct sk_buff *skb)
{
	UINT8 *ptr = NULL;
	ptr = (UINT8 *)(SKB_MAC_HEADER(skb) + 2*ETH_ALEN);

	if(*(UINT16 *)(ptr) == __constant_htons(0x8100))
		ptr += 4;
...
static unsigned int check_mcastL2L3Diff(struct sk_buff *skb)
{
	...
	struct iphdr *iph = (struct iphdr *)rtl_get_skb_ip_header(skb);	
	#endif
...
static void CheckV6UDPandU2M(struct sk_buff *pskb)
{
	...
	iph = (struct ipv6hdr *)rtl_get_skb_ip_header(pskb);
...
static void check_igmp_snooping_pkt( struct sk_buff *pskb )
{
	...
	if (IS_MCAST(dest)) 
	{
		if (unlikely((*(unsigned short *)(SKB_MAC_HEADER(pskb) + ETH_ALEN * 2) == __constant_htons(ETH_P_IP)))) 
		{
			...
			iph = rtl_get_skb_ip_header(pskb);
...
/* Hotspot 2.0 Release 1 */
	...
	if(linux_vlan_enable){
		...
		if(*((uint16*)(pskb->data+(ETH_ALEN<<1))) != __constant_htons(ETH_P_8021Q)
#if defined(CONFIG_RTL_CUSTOM_PASSTHRU)
		&& strncmp(pskb->dev->name, "pwlan", 5)!=0
#endif
			)
		{
...
#if defined(CONFIG_RTK_MESH)
		if (pskb->dev && ((strcmp(pskb->dev->name, "wlan-msh")==0)||(strcmp(pskb->dev->name, "wlan0-msh0")==0)||(strcmp(pskb->dev->name, "wlan1-msh0")==0)))
			index = 0;
		else	
#endif		
	...
	if ((!priv->pmib->dot11OperationEntry.disable_brsc) &&
		...
		!(pskb->data[0] & 0x01)
#if defined(__KERNEL__) || defined(__OSK__) 
#if defined(CONFIG_RTL_BRSHORTCUT_LINUX_VLAN_CTL) //wlan dev are not in bridge dev
		&&(linux_vlan_enable || (linux_vlan_enable==0 && br_port))
#else
		&& (br_port)
#endif
#endif
	...
	)
	{
		...
		if(pskb->data[0]==0xFF)
		{
			...
			if(*(uint16 *)&pskb->data[offset] == __constant_htons(ETH_P_ARP))
			{
				...
				if(priv->vdev_enabled==WLAN_VDEV_FOR_V6)
					tov = 0;
			}
		}else{
			//non-broadcast pkt
			...
			if(br_dev && memcmp(pskb->data, br_dev->dev_addr, MACADDRLEN)==0)
			{
				...
				//pkt not to br0
				if(priv->vdev_enabled==WLAN_VDEV_FOR_V4 || priv->vdev_enabled==WLAN_VDEV_FOR_V6)
				{
					enum ip_proto{PROTO_UNKNOWN, PROTO_IPV4, PROTO_IPV6} pkt_proto;
					uint16 proto_tmp;
					...
					if(pkt_proto==PROTO_IPV4 && priv->vdev_enabled==WLAN_VDEV_FOR_V4)
					{
						tov = 1;
					}else if(pkt_proto==PROTO_IPV6 && priv->vdev_enabled==WLAN_VDEV_FOR_V6){
						tov = 1;
					}else{
						//PROTO_UNKNOWN
						//FIXME: just keep to orignal dev, or skb_copy() to both vdev/dev?
						//printk("[%s:%d] unknown ip proto\n", __FUNCTION__, __LINE__);
						tov = 0;
...
/* Hotspot 2.0 Release 1 */
	...
	)
	{
		...
		if(L3_protocol == __constant_htons(0x8100))
			L3_protocol = *(unsigned short *)(SKB_MAC_HEADER(pskb) + MACADDRLEN*2 + 4);
...
#endif
				DRV_RT_TRACE(priv, DRV_DBG_CONN_INFO, DRV_DBG_SERIOUS, "issue_deauth - %02X:%02X:%02X:%02X:%02X:%02X, Reason: _RSON_UNSPECIFIED_\n",
								pfrinfo->sa[0],pfrinfo->sa[1],pfrinfo->sa[2],pfrinfo->sa[3],pfrinfo->sa[4],pfrinfo->sa[5]);
...
//		printk("pfrinfo->hdr_len == 0\n");
	...
	if (IS_DRV_OPEN(GET_VXD_PRIV(priv)) && isEqualMACAddr(GET_VXD_PRIV(priv)->pmib->dot11OperationEntry.hwaddr, sa))
		return 1;
#endif
#ifdef MBSSID
	if (GET_ROOT(priv)->pmib->miscEntry.vap_enable) {
...
/* ── R4/G3 Path B ─────────────────────────────────────────────────────────────
 * get_sta_nss() sat inside the enclosing `#if (BEAMFORMING_SUPPORT == 1)` block,
 * but 8192cd_sme.c calls it UNCONDITIONALLY (OnAssocReq -> pstat->mimo_num).
 * BEAMFORMING_SUPPORT is 1 only when one of CONFIG_{SLOT_0,SLOT_1,SOC}_TX_BEAMFORMING
 * is set; this build has none (SLOT_0_TX_BEAMFORMING belonged to the 8822BE slot,
 * which is deliberately compiled out), so the symbol vanished and modpost reported
 * it undefined.  The function is a pure decoder of the peer's HT/VHT capability IEs
 * — no beamforming hardware involved — so it is simply hoisted out of the guard.
 * Closing and reopening the #if is the minimal edit that does that.
 */
#endif /* BEAMFORMING_SUPPORT == 1 — reopened right after get_sta_nss() */
...
#if (BEAMFORMING_SUPPORT == 1)	/* R4/G3 Path B: reopen (see the #endif above) */
...
#if	0 //eric-8822 check csi
{
	...
	if(frtype_tmp == Type_Action_No_Ack) {
        ...
        if (rx_desc_status.CRC32) {		
			...
			priv->ext_stats.rx_reuse_frag++;
			printk("%s(%d), reuse(0x%x), pkt len=%d\n", __FUNCTION__, __LINE__, (unsigned int)priv->ext_stats.rx_reuse_frag, rx_desc_status.PKT_LEN);
			goto rx_reuse;
		}
#endif // !defined(RX_BUFFER_GATHER)
					...
					if((OPMODE & WIFI_MP_ARX_FILTER ) && (OPMODE & WIFI_MP_RX ) )
					{
						...
						if(find_flag)
						{
							printk("flag: %x\nSA: %02x:%02x:%02x:%02x:%02x:%02x\n"
									...
									"BSSID: %02x:%02x:%02x:%02x:%02x:%02x\n",
									priv->pshare->mp_filter_flag,
									sa[0],sa[1],sa[2],sa[3],sa[4],sa[5],da[0],da[1],da[2],da[3],
									da[4],da[5],bssid[0],bssid[1],bssid[2],bssid[3],bssid[4],bssid[5]);
						}
#endif
					}
					//-------------------------------------------------------------------------------------------
...
static struct rx_frinfo *defrag_frame_main(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (!GetPrivacy(pframe) && privacy != _NO_PRIVACY_)
        ...
        if (!GetPrivacy(pframe) && privacy != _NO_PRIVACY_)
        {
				...
				proto = *(unsigned short *)(pframe + hdr_len + sizeof(
				struct wlan_llc_t) + 3);
				if (proto != __constant_ntohs(0x888e)) {
                	priv->ext_stats.rx_data_drops++;
...
data_defrag_error:
	...
	if (OPMODE & WIFI_AP_STATE){
		// sta_info has freed or assoc not yet
		priv->del_sta_info.branch_code = 10;
		set_del_sta_info(priv, sa, status);
...
static int process_datafrme(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
	...
	if (OPMODE & WIFI_AP_STATE)
	{
		...
		if (is_qos_data(pframe))
		{
			if(*GetQosControl(pframe) & BIT(7)) // A-MSDU present
			{
#ifdef RTK_AC_SUPPORT
...
{
	struct rx_frinfo *pfrinfo = NULL;
	unsigned char *pframe, is_amsdu=0;
