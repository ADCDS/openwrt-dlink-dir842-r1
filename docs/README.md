...
| [`LEDS.md`](LEDS.md) | The nine panel LEDs: what drives each one, and why ours were wrong. The bootloader leaves the RTL8367S per-port LED force-mode set, so LAN1-3 were lit regardless of link while Internet and LAN4 stayed dark; `rtl8367b_setup()` now clears it. Carries the stock `gpiom.ko` `dev_leds` decode (there is exactly ONE GPIO LED on this board), the `eth_leds_ctrl` disassembly, the hardware-verified mode table, the reversed jack mapping, and a live debugfs recipe for testing LEDs without a rebuild. |
| [`RX-STALL-WEDGE.md`](RX-STALL-WEDGE.md) | The descriptor-pool wedge (issue #2 and friends). ★ Reproduce it with a **UDP** flood — TCP cannot trigger it at any rate. Two distinct failure modes, why the original FCS detector could never fire on the hard one, the new RX-stall detector, and why bridge role needed `fabric_gw_rearm=0` to use the level-3 reset without re-creating the roaming blackhole. Carries the still-open soft-degradation residue. |
